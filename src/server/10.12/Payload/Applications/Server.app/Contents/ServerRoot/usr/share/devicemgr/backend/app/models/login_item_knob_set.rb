...
#-------------------------------------------------------------------------
  ...
  def self.old_table_name;                            return 'login_item_knob_sets';              end
  def also_modifies;                                  return [:system_applications];              end
  def localized_payload_display_name(short = false);  return I18n.t('login_items_display_name');  end
  ...
  def generate_payload_hash(root_payload_identifier, profile_manager_ctx)
    payload_array = []
    add_network_share_point = hide_network_share_point = false
    @@payload_type_attributes.each_pair { |payload_type, keys|
      ...
      if payload_type == 'com.apple.loginitems.managed'
        ...
        payload_hash['AutoLaunchedApplicationDictionary-managed'] = self.system_applications.collect { |app| { 'Path' => app[:path], 'Hide' => hidden_apps.include?(app[:id]) } }
        file_arr = self.files.collect{ |file| { 'Path' => file['Path'], 'Hide' => file['Hide'] } }
        mount_arr = []
          ...
          if mount['Type'] == 'home'
            ...
            mount_arr.push({ 'AuthenticateAsLoginUserShortName' => mount['AuthenticateAsLoginUserShortName'], 'URL' => path, 'Hide' => mount['Hide'] })
          end
        } # each
      ...
      end # if payload_type == 'com.apple.loginitems.managed'
    ...
    if add_network_share_point
      ...
      payload_hash = { 'PayloadType'        => payload_type,
                       ...
                       'PayloadUUID'        => newUUID,
                       'PayloadDisplayName' => self.localized_payload_display_name_for_content(payload_type),
                       'AutoLaunchedApplicationDictionary-managed' => [ { 'AuthenticateAsLoginUserShortName' => true, 'MCX-NetworkHomeDirectoryItem' => true, 'Hide' => hide_network_share_point } ]
                      }
        payload_array.push(payload_hash)
    end # if add_network_share_point

    return payload_array
  end # generate_payload_hash

#-------------------------------------------------------------------------
end # class LoginItemKnobSet
#-------------------------------------------------------------------------
