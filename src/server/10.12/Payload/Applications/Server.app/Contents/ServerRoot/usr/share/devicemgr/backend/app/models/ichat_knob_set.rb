  ...
  def generate_payload_hash(root_payload_identifier, profile_manager_ctx)
    ...
    if attributes
      attributes.each { |attribute|
        ...
        next if value.nil?
        # possibly compose username
        value += '@' + self[:JabberHostName] if attribute == :JabberUserName && self[:JabberHostName] && self[:JabberHostName] != '' &&
                                                !value.include?('@') && !value.include?('%email%') && !value.include?('%email_address%') && !value.include?('%EMailAddress%')
