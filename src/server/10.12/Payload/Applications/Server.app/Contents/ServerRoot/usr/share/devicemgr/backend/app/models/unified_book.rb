...
#-------------------------------------------------------------------------
  ...
  def self.update_book_assignments(library_item, new_book_infos)
    ...
    book_ids = assign_book_ids.map { |id|
      ...
      "(#{id},#{li_id},#{a_mode},FALSE)"
    }
    self.connection.execute("INSERT INTO library_items_assets (asset_id, library_item_id, association_type, auto_push) VALUES #{book_ids.join(',')}") unless book_ids.empty?
    ...
    existing_book_ids.each { |id|
      ...
      self.connection.execute("UPDATE library_items_assets SET association_type = #{a_mode}, auto_push = FALSE WHERE asset_id = #{id} AND library_item_id = #{li_id}")
      changed = true
