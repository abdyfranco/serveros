...
#-------------------------------------------------------------------------
  ...
  @@payload_type_attributes = {
    ...
    'com.apple.security.firewall'    => [:EnableFirewall, :BlockAllIncoming, :EnableStealthMode],
    'com.apple.applicationaccess'    => [:allowAutoUnlock]
  }
  ...
  def self.old_table_name;  return 'privacy_knob_sets';     end
  ...
  def generate_payload_hash(root_payload_identifier, profile_manager_ctx)
    ...
    is_user_profile = library_item_type == 'UserGroup' || library_item_type == 'User' || library_item_type == 'InactiveUserGroup' || library_item_type == 'InactiveUser'
    payload_array = [];
    @@payload_type_attributes.each_pair { |payload_type, keys|
      ...
      if payload_type == 'com.apple.security.firewall'
        blocked_app_ids = (self.blockedAppIds || [])
        payload_hash['Applications'] = self.system_applications.collect { |app| { 'BundleID' => app[:identifier], 'Allowed' => blocked_app_ids.include?(app[:id]) } }
      end
  ...
  def localized_payload_display_name_for_content(type); return self.localized_payload_display_name; end
