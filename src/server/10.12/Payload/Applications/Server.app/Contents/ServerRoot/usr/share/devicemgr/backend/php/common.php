...
// Must set these first thing so we can log if we catch an error.
define('PM_LOG_DIR',            '/Library/Logs/ProfileManager');
  define('PM_PHP_LOG_FILE',     PM_LOG_DIR.'/php.log');
$gLogLevel = 4;     // Nice and high until we have gotten far enough to set it to whatever the preference says
...
define('PM_DATA_DIR',           SERVICE_DATA_LINK.'/Data');     // Only the "Data" directory is accessed through the symlink, everything else is always in PM_LIBRARY_DIR
  ...
  define('PM_IMAGES_DIR',       PM_DATA_DIR.'/Images');
...
$gClientIdent = $gClientIdentForLogging = $_SERVER['REMOTE_ADDR'];
$gLogFD       = 0;
$gLogLevel    = GetAppPreference('debugOutput', 1);
if ($gLogLevel > 1) {
  $gLogSQL = (GetAppPreference('DBLogSQL', FALSE) || GetAppPreference('DBDebug', FALSE));
  define('PLAIN',   "\x1b[0m");
...
$gClientIdentForLogging = GREEN.$gClientIdent.PLAIN;
...
$uri = (empty($_SERVER['REDIRECT_SCRIPT_URI']) ? (empty($_SERVER['SCRIPT_URI']) ? $_SERVER['REQUEST_URI'] : $_SERVER['SCRIPT_URI']) : $_SERVER['REDIRECT_SCRIPT_URI']);
//LogElapsedTime("Time since script start: %et% [$uri]", TRUE);

LogMsg('>>> Processing '.$_SERVER['REQUEST_METHOD'].' '.CYAN.$gScriptName.PLAIN, 1);
...
function CheckVPPServiceEnabled()
{
  ...
  if ($settings['vpp_service_state'] != 3)        DieServiceUnavailable('VPP service is turned off');
} // CheckVPPServiceEnabled

//-------------------------------------------------------------------------

function CheckDeviceManagementEnabled()
{
  // Make sure we are in a state where we can & should service device requests. Will not return unless we should service device requests.
  $settings = GetSettings();
  if (empty($settings['service_enabled']))                              DieServiceUnavailable('Profile Manager service is turned off');
...
function DevicemgrdSend()   // first arg is method, subsequent are args to method
{
  $args = func_get_args();
  $msg = array_shift($args);
  $msg = vsprintf($msg, $args);
  ExecuteSQLFunction('_dm_objc_send', [ 'message' => "[DMDevicemgrd $msg]" ]);
} // DevicemgrdSend
...
function DieGone($msg = '')
{
  if (!empty($msg)) $msg = ' - '.$msg;
  throw new Exception('410 Gone'.$msg);
} // DieGone
...
function FixFmt($fmt)
{
  return preg_replace('/(%(\d\$)?)@/', '\1s', $fmt);      // Replace %@ Foundation-style substitution markers with %s.
} // FixFmt
...
function LoadPHPSession()
{
  ...
  if ($gInTransaction) DieInternalError(__FUNCTION__.' must not be called from inside a database transaction');
...
function LogException($e, $level = 0)
{
  if ($e instanceof PDOException) {
    ...
    $msg = $e->getMessage();
    LogMsg(RED."EXCEPTION: $msg at\n".$e->getTraceAsString().PLAIN, $level);
  }
  return $msg;
  } // LogException
...
{
  global $gLogLevel, $gClientIdentForLogging, $gPID, $gPIDLogFD;
  if ($level > $gLogLevel) return;

  $prefix = '<'.$gClientIdentForLogging.'> ';
  if ($gLogLevel > 1) {
    $bt = debug_backtrace();
    if (isset($bt[1])) {
      $p    = $bt[1];
  ...
  dmx_log_string($level, $prefix.$str);
} // LogMsg

function LogMsgError($str)  { LogMsg(RED.$str.PLAIN, 0); }
...
function NormalizeEmail($value)
{
  ...
  $email = strtolower(preg_replace('/^.*?([^\s<>,!@#$%^&*(){}\[\]|:\"\\=]++@[^\s<>,!@#$%^&*(){}\[\]|:\'"\\=]++).*?$/', '\1', $value));
  LogMsg(__FUNCTION__.": '$value' => '$email'", 2);
...
{
  // Normalize input
  if (empty($GLOBALS['NORMALIZE_INPUTS'])) $GLOBALS['NORMALIZE_INPUTS'] = ['UDID'         => 'UDID',
                                                                           'SERIAL'       => 'SerialNumber',
...
function PrepareArrayObscuringValuesForKeys($a, $keys = NULL, $depth = 0)
{
  ...
  if (!$all_numeric) {
    foreach ($replace as $k => $v) {
          if (is_object($v))   $v = '<BinaryObject:'.get_class($v).'>';
...
function ProcessException($e, $exit = TRUE, $body = null)
{
  ...
  if ($exit) {
    ...
    $idx = strpos($errstr, "\n");
    header('Status: '.($idx === FALSE ? $errstr : substr($errstr, 0, $idx)));
...
define('DeviceSubKeysMask', 0b0000000011111111111);
define('UserSubKeysMask',   0b1111111100000000000);
...
{
  global $gStartTime, $gStatus, $gInTransaction, $gServerHeader, $gScriptName;
  ...
  $uri = (empty($_SERVER['REDIRECT_SCRIPT_URI']) ? (empty($_SERVER['SCRIPT_URI']) ? $_SERVER['REQUEST_URI'] : $_SERVER['SCRIPT_URI']) : $_SERVER['REDIRECT_SCRIPT_URI']);
  LogMsg("<<< Sent Final Output ($len bytes) - ".$_SERVER['REQUEST_METHOD'].' '.CYAN.$gScriptName.PLAIN, 1);
  LogMsg("Completed in $dur | $gStatus [$uri]");
  $GLOBALS['DB'] = null;
} // SendFinalOutput
//-------------------------------------------------------------------------

function SendFinalOutputFile($path, $len = NULL)
{
  global $gStartTime, $gStatus, $gInTransaction, $gServerHeader, $gScriptName;
  if ($gInTransaction) DieInternalError(__FUNCTION__.': There is an outstanding transaction on the database session!');
  ...
  // Let the client cache it (for one year)
