...
define('PlayLostModeSound',            1 << 48);

define('ShutDownDevice',               1 << 39);
define('RestartDevice',                1 << 38);
...
define('ValidateApplications',         1 << 25);
...
define('LastUpdateInfoCommand',        AvailableOSUpdates);  // The last command we can run as part of UpdateInformation
...
$kTaskTypeForSingleton = [ RemoveMDM                    => 'RemoveMDM',
                           ...
                           ValidateApplications         => 'UpdateInformation',
...
function Task_generate_singleton_request(&$target, $high_priority, $user_config)
{
  ...
  do {
    ...
    try {
      ...
      if ($mask & EnableLostMode) {                                                                                                               // 52
      ...
      } elseif ($mask & EnableActivationLock) {                                                                                                   // 49
        $task_type = EnableActivationLock;
        $get_next = TRUE;     // This is not a real task, just ignore it and move on.
      } elseif ($mask & PlayLostModeSound) {                                                                                                      // 48
        $task_type = PlayLostModeSound;
        if (Target_is_lab_session($target)) DieInternalError(__FUNCTION__.": Invalid task 'PlayLostModeSound' for LabSession");
        $request_hash['Command']['RequestType'] = 'PlayLostModeSound';
      ...
      } elseif ($mask & ShutDownDevice) {                                                                                                         // 39
        $task_type = ShutDownDevice;
        if (CV($target, 'IsSupervised')) {
          $request_hash['Command']['RequestType'] = 'ShutDownDevice';
        }
        else $get_next = TRUE;       // Go around again to get another task (will always clear the bit for this task type)
      } elseif ($mask & RestartDevice) {
        $task_type = RestartDevice;                                                                                                               // 38
        if (CV($target, 'IsSupervised')) {
          $request_hash['Command']['RequestType'] = 'RestartDevice';
        }
        else $get_next = TRUE;       // Go around again to get another task (will always clear the bit for this task type)
        ...
        if (!empty($pin)) $request_hash['Command']['PIN'] = $pin;
      } elseif ($mask & ValidateApplications) {                                                                                                   // 25
        $task_type = ValidateApplications;
        $request_hash['Command']['RequestType'] = 'ValidateApplications';    // This task doesn't apply to Macs
        ...
        // OS X doesn't work with lower case device id, duh!
        ...
        $user_id = CV($target, 'user_id');
        SetCVMulti($target, ['pending_checkin_token'  => GenerateUUID(),
                             'pending_user_id'        => $user_id,                    // Use the same user as we have now
                             'checkin_token_valid_at' => kDMCurrentTimestamp ]);
        if (!empty($user_id)) LabSession_create_owner_placeholder(CV($target, 'id'), $user_id);
        $task_type = RefreshMDM;
        $profile   = GenerateMDMBindingProfile($target, CreateClientCertificatePlaceholderForDevice($target));
        $request_hash['Command'] = [ 'RequestType' => 'InstallProfile',
                                     'Payload'     => new PlistData(base64_encode($profile), TRUE) ];
...
function Task_generate_install_profile_payload($profile, $target, $pkcs12)
{
  // Do we need to perform any substitutions? When we do, the profile's profile_caches holds an XML PList representation of the profile.
  // Otherwise, the data is already base64-encoded and ready to go.
  $target_type  = CV($target,  'mdm_target_type');
  $profile_data = CV($profile, 'profile_caches');
  $subKeys      = CV($profile, 'substitution_keys');
  if (empty($profile_data)) DieInternalError(__FUNCTION__.': No profile_caches for InstallProfile task');
  if (!is_array($profile_data)) DieInternalError(__FUNCTION__.': profile_caches is malformed');

  // Get the actual profile cache data by device type
      if (!empty($profile_data[$target_type]))   $profile_data = $profile_data[$target_type];
  elseif (!empty($profile_data[kDeviceTypeAny])) $profile_data = $profile_data[kDeviceTypeAny];
  else {
    LogMsg("Profile '".CV($profile, 'name')."' has no payloads applicable for '$target_type' devices.");
    return FALSE;       // No profile data for this device type, cancel the task.
  }
  ...
  $sharedDeviceFilterKeys = [ 'password',                                                                                                                                                                 // Generic
                              ...
                              'incomingpassword', 'outgoingpassword', 'smimeenabled', 'smimesigningenabled', 'smimeencryptionenabled', 'smimesigningcertificateuuid', 'smimeencryptioncertificateuuid',   // Email
                            ];
  ...
  if ($subKeys) {   // We must always run the following code if we have any substitutions as the profile cache is not base64-encoded
    ...
    if ($hash) {
      ...
      if (empty($hash['PayloadContent'])) {
        LogMsg("Profile '".CV($profile, 'name')."' has no payloads applicable for iOS user account.");
        return FALSE;         // Mark the task as failed if there are no payloads to be delivered
      }
    }
    ...
    if ($device || $user) {
      ...
      if (!empty($pkcs12)) $profile_data = SignData($profile_data, $pkcs12);
      ...
      if (!empty($pkcs12)) $profile_data = SignData($profile_data, $pkcs12);
      $profile_data = base64_encode($profile_data);
  } elseif (!empty($pkcs12)) $profile_data = base64_encode(SignData(base64_decode($profile_data), $pkcs12));

  return $profile_data;
} // Task_generate_install_profile_payload
    ...
    try {
      ...
      if (!empty($task)) {
        ...
        case 'InstallProfile':
          ...
          $pkcs12 = (Target_is_ios($target) && preg_match("/^[5-7]\.[0-9]+.*/", CV($target, 'OSVersion')) ? GetSigningIdentity() : NULL);
          $profile_data = Task_generate_install_profile_payload($profile, $target, $pkcs12);
          ...
          } else DieInternalError("No profile_caches set on library item $pid");
          break;
    ...
    if ($err && isset($err[0])) {
      ...
      if (isset($chain['ErrorCode']) && isset($chain['ErrorDomain'])) {
        if ($chain['ErrorDomain'] == 'MCMDMErrorDomain') {
          ...
          elseif ($code == 12025 || $code == 12036) return Task_handle_response($target, $incoming_request);            // ErrorCode 12025 is "App is already installed," 12036 is "The app cannot be removed while it is being installed."
        }
      }
    }
...
function Task_handle_response(&$target, $incoming_request)
{
  ...
  if (!$is_task && strcasecmp(CV($target, 'singleton_uuid'), $uuid) === 0) {
    ...
    if ($singleton != EraseDevice && $singleton != RemoveMDM && $singleton != EnableLostMode && $singleton != DisableLostMode) {   // The database handles these completely
      ...
      $ext       = NULL;
      $vals      = [ 'hp_singleton_tasks' => CV($target, 'hp_singleton_tasks'), 'lp_singleton_tasks' => CV($target, 'lp_singleton_tasks'), 'singleton_task_type' => 0 ];
      ...
      if (isset($incoming_request['ProfileList'])) {
        ...
        ExecuteSQLIntFnDeferred('_dm_bulk_auto_update_all_profiles_for_mdm_targets_throttled', $tid);
      } // if 'ProfileList'
...
SQL;
      ...
      if (!$is_ls) {
        ...
        if (isset($incoming_request['ManagedApplicationList'])) {
          ...
          if (($vals['hp_singleton_tasks'] & ValidateApplications) == 0 && Target_is_ios($target) && Target_is_at_least_version($target, '9.2')) {
            foreach ($mal as $mal_entry) {
              if (!array_key_exists('IsValidated', $mal_entry)) continue;
              $val = $mal_entry['IsValidated'];
              if (!is_bool($val) || $val) continue;
              $vals['hp_singleton_tasks'] |= ValidateApplications; // Queue up a ValidateApplications request
              break;
            }
          }
        ...
        if (isset($incoming_request['OSUpdateStatus'])) {
          ...
          if (empty($incoming_request['OSUpdateStatus'])) $vals['hp_singleton_tasks'] |= AvailableOSUpdates;
        }
        ...
        // DeviceLocation task
        if (isset($incoming_request['Longitude']))          $ext_das['Longitude']           = $incoming_request['Longitude'];
        if (isset($incoming_request['Latitude']))           $ext_das['Latitude']            = $incoming_request['Latitude'];
        if (isset($incoming_request['Altitude']))           $ext_das['Altitude']            = $incoming_request['Altitude'];
        if (isset($incoming_request['Course']))             $ext_das['Course']              = $incoming_request['Course'];
        if (isset($incoming_request['Speed']))              $ext_das['Speed']               = $incoming_request['Speed'];;
        if (isset($incoming_request['HorizontalAccuracy'])) $ext_das['HorizontalAccuracy']  = $incoming_request['HorizontalAccuracy'];
        if (isset($incoming_request['VerticalAccuracy']))   $ext_das['VerticalAccuracy']    = $incoming_request['VerticalAccuracy'];
        if (isset($incoming_request['Timestamp']))          $ext_das['Timestamp']           = $incoming_request['Timestamp'];
