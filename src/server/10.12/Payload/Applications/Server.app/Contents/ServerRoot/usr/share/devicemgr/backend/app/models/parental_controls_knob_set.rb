    ...
    @@payload_type_attributes.each_pair { |payload_type, keys|
      ...
      if payload_type == 'com.apple.familycontrols.timelimits'
        if payload_hash["familyControlsEnabled"]
          ...
          allowances = []
          if self[:weekdayTimeLimit] > 0
            allowances.push({ "fromDay"          => 0,
                              "span"             => 0,
                              "toDay"            => 4,
                              "timeLimitSeconds" => self[:weekdayTimeLimit]
                            })
          end
          if self[:weekendTimeLimit] > 0
            allowances.push({ "fromDay"          => 5,
                              "span"             => 0,
                              "toDay"            => 6,
                              "timeLimitSeconds" => self[:weekendTimeLimit]
                            })
          end
        ...
        payload_hash['filterBlacklist'].each { |url| url.sub!(/(^)/, "http://") unless url.match(/^https?:\/\//i) } if payload_hash['filterBlacklist']
        payload_hash['filterWhitelist'].each { |url| url.sub!(/(^)/, "http://") unless url.match(/^https?:\/\//i) } if payload_hash['filterWhitelist']

        # siteWhiteList is an array of dictionaries but first make sure each URL in the siteWhitelist have a scheme (i.e., http://) at the front
        if payload_hash['siteWhitelist']
          payload_hash['siteWhitelist'].each { |url| url.sub!(/(^)/, "http://") unless url.match(/^https?:\/\//i) }
          payload_hash['siteWhitelist'] = payload_hash['siteWhitelist'].collect { |url| { 'address' => url, 'bookmarkPath' => '/', 'pageTitle' => url } }
        end
