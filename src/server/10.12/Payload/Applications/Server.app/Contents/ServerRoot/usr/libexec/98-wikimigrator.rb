...
if Process.euid != 0
  puts "#{File.basename($0)} must be run as root"
  Kernel.exit!(1)
end
...
%w[ SIGHUP SIGINT SIGQUIT SIGTERM ].each { |sig| trap(sig) { Kernel.exit!(3) } }
...
if (/^10\.7/.match(sourceVersion))
  ...
  # Wiki FileData is not copied over for migration installs
