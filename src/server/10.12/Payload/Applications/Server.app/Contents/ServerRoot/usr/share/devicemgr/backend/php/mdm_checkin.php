...
require_once('/Applications/Server.app/Contents/ServerRoot/usr/share/devicemgr/backend/php/task.php');    // Needed for some of the singleton bits
...
function _checkin_transaction($incoming_request)
{
  $target = _get_checkin_target($incoming_request);
  ...
  case 'TokenUpdate':
    ...
    $bind = [ 'token'      => $incoming_request['Token']->base64EncodedData(),
              'push_magic' => $incoming_request['PushMagic'] ];
    if (!empty($incoming_request['AwaitingConfiguration'])) $bind['lp_singleton_tasks'] = (CV($target, 'lp_singleton_tasks') | DeviceConfigured);    // WARNING! DB will clear most singleton tasks at _dm_device_was_enrolled()
    SetCVMulti($target, $bind);
  ...
  case 'Authenticate':
    ...
    $vals = [];
    $das  = [];
    foreach ($incoming_request as $key => $val) {           // iOS 9 and OS X 10.10 and later include lots of useful data during Authenticate
      if ($key == 'MessageType' || $key == 'Topic' || $key == 'UDID') continue;
      if (ValidColumn($target, $key)) $vals[$key] = $val;   // If it's in the device's schema, that's where it belongs
      else                            $das[$key]  = $val;   // Otherwise, it goes in library_item_metadata.dynamic_attributes
    }
    if (!empty($vals)) {
      Device_set_unique_columns($target, $vals);            // Will leave values for non-unique columns in $vals. Also saves and re-loads $target (which might have a different 'id'!)
      SetCVMulti($target, $vals);
    }
    if (!empty($das)) {
      $ext = Target_get_extended_data($target);
      SetCVDAMulti($ext, $das);
      SaveToDatabase($ext);
    }
    ...
    $final_output = '';
    header('X-MDM-is-owned:1');                             // Always set the header for the case that the client might change default behavior
    break;
...
function _get_checkin_target($incoming_request)
{
  return Target_for_checkin_request($incoming_request);  // Will only be NULL if can't find devices row that presents a valid client certificate
} // _get_checkin_target

//-------------------------------------------------------------------------

function _save_checkin_target($args)
{
  $target = $args[0];
  $vals   = $args[1];
  // $target can be an unsaved record (LS), so check id (saved record) before reloading
  if (!EmptyCV($target, 'id')) ReloadFromDatabase($target);
  SetCVMulti($target, $vals);
  SaveToDatabase($target);
} // _save_checkin_target
...
try {
  ...
  if ($incoming_request['MessageType'] == 'UserAuthenticate') {
    $target = PerformInTransaction('_get_checkin_target', $incoming_request);
    ...
    // Handle authentication outside a transaction to avoid re-authenticating if the transaction fails
    ...
    if (!empty($auth_result['AuthToken'])) {
      $vals = ['auth_token' => $auth_result['AuthToken'], 'singleton_uuid' => NULL];
      PerformInTransaction('_save_checkin_target', [$target, $vals]);
    }
    $final_output = plist_encode_xml($auth_result);
  } else {
    $final_output = PerformInTransaction('_checkin_transaction', $incoming_request);
  }
} catch (Exception $e) {
