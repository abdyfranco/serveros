...
#-------------------------------------------------------------------------
  ...
  def self.dynamic_attributes_defaults; return { self.to_s => { :syncPeriodSeconds => 300 } };  end
  def self.old_table_name;              return 'mobility_knob_sets';                            end
  ...
  def localized_payload_display_name(short = false);  return I18n.t('mobility_display_name'); end
  ...
  def generate_payload_hash(root_payload_identifier, profile_manager_ctx)
    ...
    @@payload_type_attributes.each_pair { |payload_type, keys|
      ...
      if payload_type == 'com.apple.homeSync'
        ...
        payload_hash['excludedItems-managed']     = payload_hash.delete('excludedItems')
        payload_hash['excludedPrefItems-managed'] = payload_hash.delete('excludedPrefItems')
      end
