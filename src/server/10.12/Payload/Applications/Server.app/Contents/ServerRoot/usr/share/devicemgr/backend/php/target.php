...
define('kDeviceTypeMac',      'mac');
define('kDeviceTypeAppleTV',  'tv');

//-------------------------------------------------------------------------

function CreateClientCertificatePlaceholderForDevice($device)
{
  $settings = GetSettings();
  $devID = CV($device, 'id');
  if (empty($devID)) DieInternalError('id not set');
  $token = CV($device, 'pending_checkin_token');
  if (empty($token)) DieInternalError('pending_checkin_token not set');

  // If we have an existing row, reuse it, otherwise create a new one.
  $bind = [ 'device_id' => $devID, 'for_ota' => FALSE, 'status' => 'PENDING' ];
  $cert = FindBySQL('client_certificates', "WHERE device_id = :device_id AND for_ota = :for_ota AND status = :status", $bind);
  if (empty($cert)) $cert = NewRecordForTable('client_certificates', $bind);

  SetCVMulti($cert, [ 'checkin_token' => $token, 'scep_challenge' => base64_encode(hash('sha256', $settings['server_org_magic'].$token, TRUE)) ]);
  SaveToDatabase($cert);
  return $cert;
} // CreateClientCertificatePlaceholderForDevice

//-------------------------------------------------------------------------

function CreateClientCertificatePlaceholderForOTA($devID)
{
  if (empty($devID)) DieInternalError('id not set');
  $settings  = GetSettings();
  $token = GenerateUUID();
  ...
  // If we have an existing row, reuse it, otherwise create a new one.
  $bind = [ 'device_id' => $devID, 'for_ota' => TRUE, 'status' => 'PENDING' ];
  $cert = FindBySQL('client_certificates', "WHERE device_id = :device_id AND for_ota = :for_ota AND status = :status", $bind);
  if (empty($cert)) $cert = NewRecordForTable('client_certificates', $bind);
  ...
  SaveToDatabase($cert);
  return $cert;
} // CreateClientCertificatePlaceholderForOTA
...
function Device_update_from_query_results(&$device, $incoming_request)
{
  ...
  if (!empty($target_type)) {
    $vals['mdm_target_type'] = (stripos($target_type, 'mac') !== FALSE ? kDeviceTypeMac : (stripos($target_type, 'tv') !== FALSE ? kDeviceTypeAppleTV : kDeviceTypeiOS));
...
//  LogMsg(__FUNCTION__.'incoming_request='.PrintArrayObscuringValuesForKeys($incoming_request), 3);
  ...
  if (empty($incoming_request['UserConfiguration'])) {
    ...
    if (strcasecmp($user_guid, kiOSUserID) === 0) {
      // It's an iOS device checking in on an user channel.
      // Find the user based on the managed Apple ID in ShortName of the incoming request
      ...
      // Look for an existing LabSession for this device/user combination. If not found, create a new one. (We've already identified both device and user.)
      $session = FindBySQL('network_lab_sessions', 'WHERE device_id = :device_id AND user_id = :user_id', ['device_id' => CV($dev, 'id'), 'user_id' => CV($user, 'id')]);
      if (empty($session)) {
        $session = NewRecordForTable('network_lab_sessions');
        $setCV['device_id']       = CV($dev,  'id');
        $setCV['user_id']         = CV($user, 'id');
        $setCV['mdm_target_type'] = kDeviceTypeiOS;
      }
      $user_guid = $maid;   // Will only be used for logging after this.
    } else {
...
SQL;
      ...
      if (empty($session)) {
        ...
        $setCV['mdm_target_type'] = kDeviceTypeMac;
    ...
    } // if/else ($user_guid == kiOSUserID)
  } else {
    LogMsg(__FUNCTION__.": UserConfiguration = true", 2);
    $user_guid = '';
...
function LabSession_validate_auth_token($ls, $incoming_request)
{
  ...
  elseif (empty($incoming_request['AuthToken']) && (CV($ls, 'mdm_target_type') === kDeviceTypeiOS)) return TRUE;  // No auth validation for iOS lab sessions
...
function Target_description($target, $device = NULL)
{
  ...
  if ($is_ls) { // LabSession
    ...
    switch ($type) {
    case kDeviceTypeiOS:     $type = 'iOS'; break;
    case kDeviceTypeMac:     $type = 'Mac'; break;
    case kDeviceTypeAppleTV: $type = 'AppleTV'; break;
...
{
  return (FromTable($target, 'devices') && CV($target, 'mdm_target_type') == kDeviceTypeAppleTV);
} // Target_is_atv
...
{
  return (FromTable($target, 'devices') && CV($target, 'mdm_target_type') != kDeviceTypeMac);
} // Target_is_ios
...
{
  return (Target_is_lab_session($target) && CV($target, 'mdm_target_type') == kDeviceTypeiOS);
} // Target_is_lab_session
...
{
  return ((!$device_only && Target_is_lab_session($target)) || CV($target, 'mdm_target_type') == kDeviceTypeMac);
} // Target_is_mac
...
{
  if (!$idle && Target_is_mac($target)) return;   // macOS is much smarter about NotNow and doesn't need this hack. But on "Idle" we need to always reset skipped tasks.
...
function Target_supports_os_updates($target)
{
  ...
  if (CV($target, 'mdm_target_type') == kDeviceTypeMac) return Target_is_at_least_version($target, '10.11');
...
{
  return (FromTable($target, 'devices') && CV($target, 'mdm_target_type') != kDeviceTypeMac && CV($target, 'is_multi_user'));  // iOS devices must also be in multi user mode
} // Target_supports_multi_user_mode
