      ...
      model_commands.each { |command, params_array|
        ...
        params_array.each { |entry|
          ...
          MDMUtilities.process_in_transaction(txn_name) {
            ...
            if model_helper == KnobSetHelper
              ms = [Benchmark.ms { command_hash = model_helper.send(command, request_model, *txn_params) }, 0.001].max
            ...
            elsif txn_params.empty?
              ms = [Benchmark.ms { command_hash = model_helper.send(command) }, 0.001].max
              ms = (ms * 1000.0).round / 1000.0   # Round to 3 decimal places
              Rails.logger.info("completed in #{ms}ms") if ms >= 500.0 || MDMLogger.debugOutput?(2)
            else
              txn_params.shift if txn_params[0].nil?
              ms = [Benchmark.ms { command_hash = model_helper.send(command, *txn_params) }, 0.001].max
              ms = (ms * 1000.0).round / 1000.0   # Round to 3 decimal places
              Rails.logger.info("completed in #{ms}ms") if ms >= 500.0 || MDMLogger.debugOutput?(2)
            end
      ...
      MagicController.admin_models.each { |model_class|
        ...
        unless updated_items.empty?
          ...
          active_queries.each_pair { |guid, info|
            ...
            next unless info['method'] && (model_helper_class.to_s[0...-6] == model_str || (model_helper_class == UserHelper && model_str == 'UserGroup'))
            ...
            if query_array
              query_array.shift     # Removes the class at the head of the array of ids
              queries_hash[guid] = query_array
            else
              Rails.logger.error("query_result[:remote][#{guid}] = #{query_result[:remote][guid.to_sym]}")
              queries_hash[guid] = []
            end
