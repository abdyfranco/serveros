...
#-------------------------------------------------------------------------
  ...
  def add_communication_service_handlers(payload_hash)
    
    audio_handler_info = payload_hash.delete('AudioCall')
    payload_hash['CommunicationServiceRules'] = {
      'DefaultServiceHandlers' => {'AudioCall' => audio_handler_info['bundleId']} 
    } if audio_handler_info

    return payload_hash
  end # add_communication_service_handlers
