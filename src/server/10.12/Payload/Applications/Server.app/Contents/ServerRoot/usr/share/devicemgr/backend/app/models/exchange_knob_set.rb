...
#-------------------------------------------------------------------------
  ...
  def modify_attributes(attr_hash, extended = false)
    ...
    attr_hash[:payload_type] = self.payload_type
    return attr_hash
  ...
  def modify_payload_hash(payload_hash)
    ...
    # Add the old SMIMEEnabled key if either of the new SMIME keys are enabled (for older clients)
    payload_hash["SMIMEEnabled"] = true if payload_hash["SMIMESigningEnabled"] || payload_hash["SMIMEEncryptionEnabled"]
