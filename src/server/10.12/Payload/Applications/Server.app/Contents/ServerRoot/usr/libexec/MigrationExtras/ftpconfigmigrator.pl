...
#                  Time Machine backup.
# --sourceVersion <ver> The version number of the old system (like 10.6 or 10.11). Since we support migration from 10.6 - 10.11 installs.
...
#
# Example: /Applications/Server.app/Contents/ServerRoot/usr/libexec/MigrationExtras/ftpconfigmigrator.pl --purge 0 --language en --sourceType System --sourceVersion 10.6 --sourceRoot "/Library/Server/Previous" --targetRoot "/"
...
#10.6.x and 10.11.x stores the state of the service under launchd in the following file.
$LAUNCHD_OVERRIDES = "/private/var/db/launchd.db/com.apple.launchd/overrides.plist";
#New server stores this state under a different location.
$SERVERD_PLIST = "/Library/Server/Setup/com.apple.serverd.plist";
...
$MINVER="10.6"; # => 10.6
$MAXVER="10.11.0"; # <  10.11.0

$FTP_DISABLED="true"; #To begin with...
$FTP_DISABLED_STATE="true"; #Initialize to true (which is disabled) in case we can't read the state, due to missing file, etc.
...
##################   Functions   #######################
##
sub getPriorServiceState()
{
	if (${FUNC_LOG}) {printf("getPriorServiceState := S\n");}
	if ((${SRV_MINOR} eq "6") || (${SRV_MINOR} eq "7") || (${SRV_MINOR} eq "8") || (${SRV_MINOR} eq "9") || (${SRV_MINOR} eq "10")) {
		if (-e ${OLD_LAUNCHD_OVERRIDES}) {
			if (${DEBUG}) {
				print("cmd := " . qq(${PLISTBUDDY} -c \"Print :com.apple.xftpd:Disabled\" \"${OLD_LAUNCHD_OVERRIDES}\") . "\n");
			}
			$FTP_DISABLED_STATE=qx(${PLISTBUDDY} -c \"Print :com.apple.xftpd:Disabled\" \"${OLD_LAUNCHD_OVERRIDES}\" 2>&1);
			$CallResult=$?; chomp($CallResult);
			if ( $CallResult ne "0" ) {		#This means the key was absent and we failed to read it out of the overrides plist.
				$FTP_DISABLED_STATE="true"
			}
			chomp(${FTP_DISABLED_STATE});
		}
		#The entry could be present+enabled, or present+disabled, or just plan missing!
		if ( (${FTP_DISABLED_STATE} eq "true") ) {
			$FTP_DISABLED = "true";
	...
	#In modern server the setting for FTP service is stored in 
	## /Library/Server/Setup/com.apple.serverd.plist
	if (-e ${OLD_SERVERD_SERVICES}) {
		$path = NSString->stringWithCString_($SERVERD_PLIST);
		$dict = NSDictionary->dictionaryWithContentsOfFile_($path);
		$key = NSString->stringWithCString_("dynamicServices");
		$allServices = $dict->objectForKey_($key);
		$disabled=1;
		
		$enumerator = $allServices->objectEnumerator();
		while ($tKey = $enumerator->nextObject() and $$tKey) {
			$tURL=$tKey->description()->cString();
			if ($tKey->objectForKey_("Label")->description()->cString() =~ "com.apple.servermgrd.ftp") {
				if ($ENV{DEBUG} eq 1) {
					printf "curDict := %s\n", $tKey->description()->cString();
					printf "curDict:Label := %s\n", $tKey->objectForKey_("Label")->description()->cString();
					printf "curDict:Label := %d\n", $tKey->objectForKey_("Disabled")->description()->cString();
				}
				$disabled = $tKey->objectForKey_("Disabled")->description()->cString();
				chomp($disabled);
				if ($disabled =~ "0") {
					$FTP_DISABLED = "false";
				}
	...
	if (${FUNC_LOG}) {printf("getPriorServiceState := E\n");}
}
...
sub startStopFTP()
{
	...
	if(${FTP_DISABLED} eq "true") {
		qx(${SERVERCTL} disable service=com.apple.ftpserver 2>&1);
		if (${DEBUG}) { printf("%s\n", qq(${SERVERCTL} disable service=com.apple.ftpserver)); }
	} else {
		qx(${SERVERCTL} enable service=com.apple.ftpserver 2>&1);
		if (${DEBUG}) { printf("%s\n", qq(${SERVERCTL} enable service=com.apple.ftpserver)); }
	}
		...
		if( ($mu->pathExists("${gSourceRoot}")) && ($mu->pathExists("${gTargetRoot}")) ) {
			if ($mu->isValidLanguage($gLanguage)) {
				if ($mu->isValidVersion($gSourceVersion)) {
					...
					$OLD_SERVERD_SERVICES = "${gSourceRoot}" . "${SERVERD_PLIST}";
					if (${DEBUG}) {
...
sub Usage()
{
	...
	print("--sourceVersion <ver> The version number of the old system (like 10.6, 10.7, 10.8 - 10.11). Since we support migration from 10.6 - 10.11." . "\n");
