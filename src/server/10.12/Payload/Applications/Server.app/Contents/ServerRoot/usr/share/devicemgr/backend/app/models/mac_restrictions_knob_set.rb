...
#-------------------------------------------------------------------------
  ...
  @@valid_payload_types = [ 'com.apple.NetworkBrowser',
  ...
  @@payload_type_attributes = {
    ...
    'com.apple.desktop'               => [],
    'com.apple.MCX'                   => [:safariAllowAutoFill],
    'com.apple.coremediaio.support'   => [],
    'com.apple.applicationaccess'     => [:allowCloudDocumentSync, :allowCloudDesktopAndDocuments, :allowSpotlightInternetResults, :allowCloudKeychainSync, :allowMusicService, :allowCloudMail, :allowCloudAddressBook, :allowCloudCalendar, :allowCloudReminders, :allowCloudBookmarks, :allowCloudNotes, :allowFingerprintForUnlock, :allowDefinitionLookup],
    'com.apple.preferences.users'     => [:DisableUsingiCloudPassword]
  }
  ...
  def self.old_table_name;                            return 'mac_restrictions_knob_sets';                                      end
  def self.dynamic_attributes_defaults;               return { self.to_s => { :restrict_store_softwareupdate_only => false } }; end
  def also_modifies;                                  return [:system_applications, :widgets];                                  end
  def localized_payload_display_name(short = false);  return I18n.t('restrictions_display_name');                               end
  def self.valid_payload_types;                       return @@valid_payload_types;                                             end
  ...
  end # localized_payload_display_name_for_content
  ...
  def generate_payload_hash(root_payload_identifier, profile_manager_ctx)
    ...
    @@payload_type_attributes.each_pair { |payload_type, keys|
      ...
      when 'com.apple.applicationaccess.new'
         ...
         # include whiteList only if there are items in it, setting an empty whitelist blacklists all apps.
         if system_applications.length > 0
      ...
      # *** System Preferences ***
      when 'com.apple.systempreferences'
        payload_hash[self[:internal_use_flag_BlacklistPrefPanes] ? 'DisabledPreferencePanes' : 'EnabledPreferencePanes'] = self[:EnabledPreferencePanes] if self[:RestrictPrefPanes]
      ...
      when 'com.apple.coremediaio.support'
        payload_hash['Device Access Allowed'] = self[:deviceAccessAllowed] if !self[:deviceAccessAllowed].nil?

      end # case

      payload_array.push(payload_hash)
    } # @@payload_type_attributes.each_pair
    return { 'mac' => payload_array }
  end # generate_payload_hash

#-------------------------------------------------------------------------
end # MacRestrictionsKnobSet
#-------------------------------------------------------------------------
