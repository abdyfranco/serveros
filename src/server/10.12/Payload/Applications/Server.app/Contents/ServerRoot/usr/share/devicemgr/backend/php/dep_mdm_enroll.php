...
function _dep_mdm_enroll_2(array $args)
{
  ...
  if (!empty($args['userData'])) {
    ...
    if ($device) {
      if (CV($device, 'is_dep_device')) {
        ...
        $check_ac = FALSE;    // DEP devices should *never* be enrolling via AC mechanisms if they have a DEP profile set
        if ($permissions->enroll_dep) {
          if (!empty($args['has_dep_profile'])) {   // A DEP device must have a DEP profile defined for it somewhere
            ...
            $check_ac = $permissions->enroll_configurator;
            $perms_result = 'Device has no DEP profile assigned';
          }
  ...
  if (empty($device)) $device = NewRecordForTable('devices', [ 'SerialNumber' => $args['SERIAL'] ]);
  ...
  if (Target_is_mac($device)) LabSession_create_owner_placeholder(CV($device, 'id'), $user_id);
...
function _find_or_create_user(&$args)
{
  ...
  if (!$user) {
    ...
    } else $node_id = CV($od_node, 'od_node_id');
    ...
    $user = NewRecordForTable('users', $set);
    SaveToDatabase($user);
  }
  return $user;
...
function _request_authorization($udid)
{
  ...
  $gAuthPrompt = sprintf(FixFmt(I18n_t('dep_prompt_credentials')), $org);
...
try {
  ...
  $final_output = GenerateMDMBindingProfile($results['device'], $results['cert']);
} catch (Exception $e) {
