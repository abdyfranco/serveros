...
/Applications/Server.app/Contents/ServerRoot/usr/sbin/postconf -c /Library/Server/Mail/Config/postfix -e imap_submit_cred_file=/Library/Server/Mail/Config/postfix/submit.cred
...
if [ ! -e /Library/Server/Mail/Config/postfix/submit.cred ] ; then
	hostname=`/Applications/Server.app/Contents/ServerRoot/usr/sbin/postconf -c /Library/Server/Mail/Config/postfix -h myhostname`
	if [ ! "$hostname" ] ; then
		hostname=`hostname`
	fi
