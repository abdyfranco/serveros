...
CC.WikiEditor.EditorController = Class.create(CC.Keyboard.Mixins.Responder, {
	...
	toggleEditMode: function(inOptEditModeEnabled, inOptDoNotCheckForNewerVersion, inOptPageSaveErrorCondition) {
		...
		// Check that the user is logged in in the background
	...
	handleWindowMouseUp: function(inEvent) {
		...
		// get new/current text selection
		var selection = null;		
		...
		// if text selection is not null then cache current selection		
		if (selection.anchorNode) {
			...
			textSelectionDelegate.cacheSafariSelection(cachedSelection);			
		}
	},
...
CC.WikiEditor.EditorToolbarView = Class.create(CC.Mvc.View, {
	render: function() {
		...
		// Add specific accessibility for iPad
		var params;
		if (browser().isMobileSafari()) {
			params = {className: 'contents', role: 'link', 'aria-label': "_Accessibility.group.editToolBar".loc()};
		} else {
			params = {className: 'contents'};
		}
				
		var toolbar = Builder.node('div', {className: 'editor toolbar chrome'}, [
			Builder.node('div', params, [
				...
				// XXX buttons are temporarily disabled from tabindexing (will be handled when we'll make the editor accessible as well)
