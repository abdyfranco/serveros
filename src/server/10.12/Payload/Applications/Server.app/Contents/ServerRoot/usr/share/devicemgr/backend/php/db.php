...
define('kDMCurrentTimestamp', "(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')");
define('kPGEpochTimestamp',   'epoch');
define('kMaxConflict',        100);     // Put an upper limit on the number of retries, lest we find ourselves in some kind of infinite loop
define('kMaxDeadlock',        10);      // Deadlocks are painful (and take at least 1 second to detect), so we don't allow as many before dying
define('kConflictDelayFloor', 1000);    // 1ms    Delay a random amount, min of 1ms, max of 1ms plus 4ms per retry attempt
define('kConflictDelayEach',  4000);    // 4ms
define('kDeadlockDelayFloor', 100000);  // 100ms  Delay a random amount, min of 100ms, max of 100ms plus 50ms per retry attempt
define('kDeadlockDelayEach',  50000);   // 50ms

define('kDMPGPriorityLowest',  0);
define('kDMPGPriorityLower',   1);
define('kDMPGPriorityLow',     2);
define('kDMPGPriorityDefault', 3);
define('kDMPGPriorityHigh',    4);
define('kDMPGPriorityHigher',  5);
define('kDMPGPriorityHighest', 6);
...
try {
  if (function_exists('apc_fetch')) {
    ...
    } else $GLOBALS['gTableSchemas'] = $schema;
  } else require_once('/Applications/Server.app/Contents/ServerRoot/usr/share/devicemgr/backend/php/schema.php');
...
function ExecuteSQLFunction($fn, $params)
{
  ...
  $args = '';
  foreach ($gTableSchemas[$fn] as $arg => $type) {    // Process the arguments in the order expected by the function
    if (!array_key_exists($arg, $params)) break;      // Once we have no input argument, the remaining parameters must have default values, so we're done
...
function ExecuteSQLIntFnDeferred($fn, $intval)
{
  return ExecuteSQLFunction('_dm_defer_batch_int_function_call', [ 'fn' => $fn, 'intval' => $intval ]);
} // ExecuteSQLIntFnDeferred
...
function IsTransactionFailure(Exception $e)
{
  ...
  // Any "Class 40" error code is a transaction-related failure (including 40P01 "deadlock_detected")
  $state = $e->getCode();
  ...
  $msg = $e->getMessage();
  if (strlen($msg) > 0 && (stripos($msg, 'could not serialize') !== FALSE || stripos($msg, 'deadlock detected') !== FALSE)) return TRUE;
  return FALSE;
...
function LogPDOException($e, $st = NULL, $level = 0, $query = '')
{
  ...
  } else LogMsg("PDOException: '$msg' (info: '$info', code:$code)", 4);       // Log the transaction failures at 4 to keep them out of all but the most verbose logs
  return $msg;
} // LogPDOException
...
function NewRecordForTable($table, $values = NULL)
{
  global $gTableSchemas;
  ...
  if (is_array($values) && !empty($values)) _set_multiple_cv($row, $values, FALSE);
  return $row;
} // NewRecordForTable

// --------------------------------------------------------------------

function PerformInTransaction($fn, $args = NULL, $priority = NULL, $mode = 'SERIALIZABLE')
{
  global $gLogLevel, $gPID;

  if (is_null($priority)) $priority = kDMPGPriorityDefault;
  $connID = "P:$gPID";
  $txnID  = "$fn";
  LogMsg(__FUNCTION__.": connID=$connID, txnID=$txnID", 3);
  $numConflict = $numDeadlock = 0;
  $original_level = $gLogLevel;
  do {
    $retry = FALSE;
    dmx_will_begin_transaction($connID, $txnID, $priority);
    BeginTransaction($mode);
    try {
      if (!empty($args)) $result = $fn($args);
      else               $result = $fn();
      CommitTransaction();
      dmx_did_complete_transaction($connID, TRUE, FALSE);
      ...
      RollbackTransaction();
      if (!$retry) {
        dmx_did_complete_transaction($connID, FALSE, FALSE);
      ...
      if ($e->getCode() == '40P01') {     // Deadlock!
        $numDeadlock += 1;
        if ($numDeadlock > kMaxDeadlock) {
          dmx_did_complete_transaction($connID, FALSE, FALSE);
          $gLogLevel = $original_level;
          DieInternalError(__FUNCTION__.": Giving up after $numDeadlock transaction deadlocks.");
        }

        $delay = kDeadlockDelayFloor + mt_rand(0, $numDeadlock * kDeadlockDelayEach);
        LogMsg("@@@ Retry #$numDeadlock of '$fn' due to database transaction deadlock '$msg'.... (in ".($delay/1000.0)."ms) @@@", 0);
        if ($gLogLevel < 4) $gLogLevel += 1;  // Increase the log level after every deadlock, to a max of level 4
      } else {
        $numConflict += 1;
        if ($numConflict > kMaxConflict) {
          dmx_did_complete_transaction($connID, FALSE, FALSE);
          $gLogLevel = $original_level;
          DieInternalError(__FUNCTION__.": Giving up after $numConflict transaction conflicts.");
        }

        $delay = kConflictDelayFloor + mt_rand(0, $numConflict * kConflictDelayEach);
        LogMsg("@@@ Retry #$numConflict of '$fn' due to database transaction conflict '$msg'.... (in ".($delay/1000.0).'ms) @@@', ($numConflict % 5 == 0 ? 1 : 4));
        if ($numConflict % 5 == 0 && $gLogLevel < 4) $gLogLevel += 1;  // Increase the log level after every 5 retries, to a max of level 4
      }
      // usleep($delay);
      dmx_did_complete_transaction($connID, FALSE, $retry);
    } // try/catch
  } while ($retry);
  $gLogLevel = $original_level;
...
function UnsetCVDA(&$values, $col)
{
  UnsetCVDAMulti($values, [$col]);
} // UnsetCVDA

//-------------------------------------------------------------------------

function UnsetCVDAMulti(&$values, $cols)
{
  $da = CV($values, 'dynamic_attributes');
  if (!empty($da) && is_array($cols)) {
    foreach ($cols as $key) unset($da[$key]);
  }
  SetCV($values, 'dynamic_attributes', $da);
} // UnsetCVDAMulti
  ...
  if ($type == 'array' || $type == 'hash') {
    ...
    if (!array_key_exists($usv_key, $values)) { // We haven't unserialized this value yet
      if (!empty($values[$col])) {              // Make sure there's something to unserialize
        if ($type == 'array') $values[$usv_key] = ParseJSON($values[$col], FALSE);
        else $values[$usv_key] = ParseJSON($values[$col]);
        $val = $values[$usv_key];
    ...
    if ($val !== NULL) {
      ...
      case 'hash':
        ...
        if (!empty($val)) {
          ...
          if (empty($json)) {
            $val = ($type == 'array' ? '[]' : '{}');      // Empty array or hash in JSON, as appropriate
            LogMsg("json_encode failed for '$key' ($type), writing $val\n".PrintArrayObscuringValuesForKeys($val));
          } else $val  = $json;
        } elseif (is_null($val)) {
          $val  = NULL;
          $type = PDO::PARAM_NULL;
        } else $val = ($type == 'array' ? '[]' : '{}');      // Empty array or hash in JSON, as appropriate
        break;
