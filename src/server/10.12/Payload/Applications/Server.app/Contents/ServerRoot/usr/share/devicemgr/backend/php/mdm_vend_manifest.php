...
function _vend_manifest_transaction($uuid)
{
  ...
  if (empty($icon) || !file_exists(PM_FILE_STORE_DIR."/$icon")) {
    $icon = (($supp & 16) ? 'generic_app_tv.png' : 'generic_app.png');                      // Fallback to a generic icon
  }
  $assets   = [ $pkg_hash,
                [ 'kind'        => 'display-image',
                  'needs-shine' => FALSE,
...
try {
  ...
  $result = PerformInTransaction('_vend_manifest_transaction', $uuid);
