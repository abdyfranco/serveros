...
class LoginWindowKnobSet < KnobSet
  ...
  @@payload_type_attributes = {
    ...
    'com.apple.screensaver' => [:loginWindowIdleTime, :loginWindowModulePath, :moduleName],
    'com.apple.MCX'         => [:DisableGuestAccount],
    ...
    'com.apple.SetupAssistant.managed' => [:SkipCloudSetup]
  }
    ...
    @@payload_type_attributes.each_pair { |payload_type, keys|
      ...
      keys.each{ |attribute|
        ...
        if attribute == :AutoLogOutDelay
          payload_hash["com.apple.autologout.AutoLogOutDelay"] = self[attribute] if self[attribute] > 0
          ...
          if self[attribute.to_s]
            ...
            users.each{ |x| payload_hash["AllowList"].push(x["guid"].upcase) unless x["guid"].empty? }
          end
        elsif attribute == :AllowUserGroups
          payload_hash["AllowList"] ||= []
          if self[attribute.to_s]
            groups = UserGroup.get_attributes_for_multiple_by_id(self[attribute.to_s], true)
            groups.each{ |x| payload_hash["AllowList"].push(x["guid"].upcase) unless x["guid"].empty? }
          end
        elsif attribute == :DenyUsers
          payload_hash["DenyList"] ||= []
          if self[attribute.to_s]
            users = User.get_attributes_for_multiple_by_id(self[attribute.to_s], true)
            users.each{ |x| payload_hash["DenyList"].push(x["guid"].upcase) unless x["guid"].empty? }
          end
        elsif attribute == :DenyUserGroups
          payload_hash["DenyList"] ||= []
          if self[attribute.to_s]
            groups = UserGroup.get_attributes_for_multiple_by_id(self[attribute.to_s], true)
            groups.each{ |x| payload_hash["DenyList"].push(x["guid"].upcase) unless x["guid"].empty? }
          end
        elsif attribute == :loginWindowIdleTime
          payload_hash["loginWindowIdleTime"] = value
          payload_hash["idleTime"] = value
