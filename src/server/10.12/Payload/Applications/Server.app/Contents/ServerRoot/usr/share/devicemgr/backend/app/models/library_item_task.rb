...
#-------------------------------------------------------------------------
  ...
  def self.create(library_item_target, task_type, parameters = nil, guid = nil)
    ...
    if @@collected_tasks
      @@collected_tasks.add([library_item_target, task_type, parameters, guid])
      return nil
    end
    ...
    if task_type == 'DeleteUser'
      ...
      sql = "SELECT dm_create_delete_user_task_for_library_item(#{library_item_target.id}, #{json}::json, #{guid}::uuid) AS id"
      ...
      guid = (guid ? self.connection.quote(guid) : 'NULL')
      sql = "SELECT dm_create_basic_task_for_library_item(#{library_item_target.id}, '#{task_type}', #{json}::json, #{guid}::uuid) AS id"
      results = self.connection.select_integer_values_by_index(sql)
    end
