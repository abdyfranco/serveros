...
#-------------------------------------------------------------------------
  ...
  def self.destroy(id)
    ...
    return_hash[owner.library_item_type.to_s.downcase] = { :updated => [owner.get_attributes] } if owner
    return return_hash
  end # self.destroy
  ...
  def self.get_raw_profile(library_item_id)
    profile = Profile.find_by_library_item_id(library_item_id)
    return (profile ? ProfileManager::generate_raw_profile(profile) : nil)
  end # self.get_raw_profile

  #-------------------------------------------------------------------------

  def self.get_signed_profile(library_item_id, hostname)
    profile = Profile.find_by_library_item_id(library_item_id)
    return (profile ? ProfileManager::generate_signed_profile(profile) : nil)
  end # self.get_signed_profile
