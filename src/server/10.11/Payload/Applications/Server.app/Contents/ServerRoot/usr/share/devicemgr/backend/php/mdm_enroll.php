...
function _enroll_transaction($args)
{
  ...
  SetCVMulti($device, [ 'pending_user_id'        => $user_id,
                        ...
                        'mdm_target_type'        => ($is_mac ? 'mac' : 'ios')
                      ]);
...
try {
  ...
  PerformInTransaction('_enroll_transaction', $args, kDMPGPriorityLow);

  $final_output = GenerateMDMBindingProfile($mdm_acl, $checkin_token);
