...
CC.EntityFavoriteToggle = Class.create(CC.BaseEntityToggle, {
	persistIsSelected: function() {
		var selectedGuid = this.element.getDataAttributes().guid;
		
		// iPad: Making sure we're selecting the document guid to apply favorite/unfavorite
		if (browser().isMobileSafari() && !selectedGuid) {
			selectedGuid = this.element.up('.editor').down('.page').getAttribute('data-guid');
		}
		
		if (this.getIsSelected()) {
			server_proxy().addEntityToFavorites(selectedGuid, function() {
				globalNotificationCenter().publish(CC.DID_FAVORITE_ENTITY_NOTIFICATION, this, {'guid': this.element.getDataAttributes().guid});
			}.bind(this));
		} else {
			server_proxy().removeEntityFromFavorites(selectedGuid, function() {
...
var TagSearchField = Class.create(UserSearchField, {
	...
	constructQuery: function(inSearchString) {
		...
		if (inSearchString.length != 0) {
			server_proxy().allTagsStartingWith(inSearchString, function(results){ 
				var data = [];
				// mapping old style (basic get/json) with new style (server_proxy responses)
				results.each(function(row) { 
					data.push({ entityGUID: row, displayString: row, url: "#"});
				});				
				this.mRows = data;				
				this.draw();
				this.mTimer = null;
