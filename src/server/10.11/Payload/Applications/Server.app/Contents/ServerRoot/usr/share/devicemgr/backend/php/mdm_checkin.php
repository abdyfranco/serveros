...
function _checkin_transaction($args)
{
  ...
  $owned  = FALSE;
  if ($target) {
    if (FromTable($target, 'devices')) {
      $act = CV($target, 'active_checkin_token');
      $pct = CV($target, 'pending_checkin_token');
      if (!empty($act) && $act != $pct) $owned = !EmptyCV($target, 'user_id');
      elseif (!empty($pct))             $owned = !EmptyCV($target, 'pending_user_id');
    } elseif (Target_is_lab_session($target)) $owned = CV($target, 'is_owner');
  }
  ...
  case 'TokenUpdate':
    ...
    if (!empty($incoming_request['AwaitingConfiguration'])) {
      $bind['lp_singleton_tasks'] = (CV($target, 'lp_singleton_tasks') | DeviceConfigured);    // WARNING! DB will clear most singleton tasks at _dm_device_was_enrolled()
    }
