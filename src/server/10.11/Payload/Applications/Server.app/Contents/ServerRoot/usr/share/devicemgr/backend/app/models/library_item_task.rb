...
#-------------------------------------------------------------------------
  ...
  def self.create(library_item_target, task_type, parameters = nil, guid = nil, device_ids = nil)
    ...
    targets = (device_ids && device_ids.collect { |id| Integer(id) })  # Sanitize the input, make sure they're valid integers
    results = nil

    # Handle delete user separately to parse incoming params and then calling dm_create_delete_user_task_for_library_item to create tasks
    if task_type == 'DeleteUser'
      users = parameters.delete('users')
      params = nil
      params = users.reduce Hash.new, :merge unless users.empty?   # Converts an array of hashes to hash  [{"name1"=>true}, {"name2"=>false}] => {"name1"=>true, "name2"=>false}
      # users.each { |u| u.map { |k,v|  params[k] = v } }
      # params = "{ #{params.join(',')} }" unless params.empty?

      json = (params ? "#{self.connection.quote(params.generate_internal_JSON)}" : 'NULL')
      guid = (guid ? self.connection.quote(guid) : 'NULL')
      targets = (targets ? "ARRAY[#{targets.join(',')}]" : 'NULL')
      sql = "SELECT dm_create_delete_user_task_for_library_item(#{library_item_target.id}, #{json}::json, #{guid}::uuid, #{targets}::integer[]) AS id"
      ...
      if library_item_target.is_a?(Profile)
        ...
        names = parameters.delete('admin_accounts')
        unless names.empty?
          names.map! { |n| self.connection.quote(n) }
          join    = 'library_item_metadata AS m USING (id)'   # JOIN to library_item_metadata to access dynamic_attributes
          where   = "d.is_dep_device AND d.mdm_target_type = 'mac' AND (m.dynamic_attributes#>>'{AutoSetupAdminAccounts,0,shortName}') IN (#{names.join(',')})"
          aa_ids  = library_item_target.get_all_device_ids(where, join)
          targets = (targets.empty? ? aa_ids : targets & aa_ids)
        end
      end
      ...
      guid = (guid ? self.connection.quote(guid) : 'NULL')
      targets = (targets ? "ARRAY[#{targets.join(',')}]" : 'NULL')
      sql = "SELECT dm_create_basic_task_for_library_item(#{library_item_target.id}, '#{task_type}', #{json}::json, #{guid}::uuid, #{targets}::integer[]) AS id"
      results = self.connection.select_integer_values_by_index(sql)
    end
