      ...
      unless self.method_defined?(:get_all_device_ids_for_task)  # Don't override any existing implementation
        def get_all_device_ids_for_task(task_type)
          ...
          when 'ClearPasscode'
            ...
            where = "d.mdm_target_type <> 'mac' AND (m.dynamic_attributes->>'unlock_token') IS NOT NULL"

          # ios only - 8.0 and later
          when 'ClearRestrictionsPassword'
            where = "d.mdm_target_type <> 'mac' AND d.\"IsSupervised\" AND (\"OSVersion\" ~* '[8-9]\..*' OR \"OSVersion\" ~* '[1-9][0-9]\..*')"

          # os x 10.10 and greater, ios 8.0 and greater, supervised only
          when 'SetDeviceName'
            ios_where = "d.mdm_target_type <> 'mac' AND d.\"IsSupervised\" AND (\"OSVersion\" ~* '[8-9]\..*' OR \"OSVersion\" ~* '[1-9][0-9]\..*')"
            osx_where = "d.mdm_target_type = 'mac' AND \"OSVersion\" ~* '10\.[1-9][0-9].*'"
            where = "(#{ios_where}) OR (#{osx_where})"

          when 'PushOSXEnterpriseApplications'
            where = "d.mdm_target_type = 'mac' AND \"OSVersion\" ~* '10\.[1-9][0-9].*'"

          when 'AllowActivationLock'
            where = "d.mdm_target_type <> 'mac' AND d.\"IsSupervised\""
          when 'RemoveActivationLock'
            where = "d.mdm_target_type <> 'mac' AND d.\"IsSupervised\" AND d.activation_lock_bypass_code IS NOT NULL"
          ...
          when 'EnableLostMode'
            join  = 'library_item_metadata AS m USING (id)'   # JOIN to library_item_metadata to access dynamic_attributes
            where = "d.mdm_target_type <> 'mac' AND d.\"IsSupervised\" AND d.\"OSVersion\" ~* '^(9\.[3-9]|[1-9]\d+)' AND (m.dynamic_attributes->>'IsMDMLostModeEnabled') IS DISTINCT FROM 'true'"
          ...
          when 'DeviceLocation', 'DisableLostMode'
            join  = 'library_item_metadata AS m USING (id)'   # JOIN to library_item_metadata to access dynamic_attributes
            where = "d.mdm_target_type <> 'mac' AND d.\"IsSupervised\" AND d.\"OSVersion\" ~* '^(9\.[3-9]|[1-9]\d+)' AND (m.dynamic_attributes->>'IsMDMLostModeEnabled') = 'true'"
          ...
          when 'EnableAppAnalytics'
            join  = 'library_item_metadata AS m USING (id)'   # JOIN to library_item_metadata to access dynamic_attributes
            where = "d.is_multi_user AND d.\"OSVersion\" ~* '^(9\.[3-9]\.[1-9]|[1-9]\d+)' AND (m.dynamic_attributes->>'DiagnosticSubmissionEnabled') = 'true' AND (m.dynamic_attributes->>'AppAnalyticsEnabled') IS DISTINCT FROM 'true'"

          when 'DisableAppAnalytics'
            join  = 'library_item_metadata AS m USING (id)'   # JOIN to library_item_metadata to access dynamic_attributes
            where = "d.is_multi_user AND d.\"OSVersion\" ~* '^(9\.[3-9]\.[1-9]|[1-9]\d+)' AND (m.dynamic_attributes->>'DiagnosticSubmissionEnabled') = 'true' AND (m.dynamic_attributes->>'AppAnalyticsEnabled') = 'true'"

          when 'PasscodeLockGracePeriod'
            where = "d.is_multi_user AND d.\"OSVersion\" ~* '^(9\.[3-9]\.[1-9]|9\.[4-9].*|[1-9]\d+)'" # iOS 9.3.1+
          end # case task_type
      ...
      unless self.method_defined?(:get_os_updates_info)  # Don't override any existing implementation
        def get_os_updates_info
          ...
          info.each { |i|
            ...
            i['Title'] = (names[locale] || names['en'] || names['DEFAULT'] || '') if names
          } # info.each
          return info
        end
      end # unless self.method_defined?
