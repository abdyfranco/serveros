...
#-------------------------------------------------------------------------
  ...
  def all_ancestor_device_groups
    sql = <<-SQL
      ...
      JOIN   view_device_groups_flat AS j ON (j.parent_id = g.id)
  ...
  def all_child_device_groups
    sql = <<-SQL
      SELECT g.*
      FROM   #{self.table_name}      AS g
      JOIN   view_device_groups_flat AS j ON (j.child_id = g.id)
  ...
  def get_all_ios_apps_info
    sql = <<-SQL
      ...
      FROM   installed_ios_applications      AS ia
  ...
  def get_all_profiles
    sql = <<-SQL
      ...
      UNION
        ...
        FROM   #{Profile.table_name}   AS p
        JOIN   view_device_groups_flat AS j ON (p.library_item_id = j.parent_id)
        WHERE  j.child_id = #{self.id}
    SQL
  ...
  def get_app_data;                         return (self.connection.select_json_value_by_index("SELECT dm_app_info_for_device_group(#{self.id})") || []);                                        end
