...
#-------------------------------------------------------------------------
  ...
  def self.update(edu_class_id, incoming_request)
    ...
    edu_class_students     = incoming_request.delete('students')          || []
    edu_class_students    += incoming_request.delete('student_groups')    || []
    edu_class_instructors  = incoming_request.delete('instructors')       || []
    edu_class_instructors += incoming_request.delete('instructor_groups') || []
    edu_class_devices      = incoming_request.delete('device_groups')     || []

    edu_class.update_from_hash(incoming_request)
    edu_class.save

    edu_class.update_edu_class_students_and_instructors(edu_class_students, edu_class_instructors)
    edu_class.update_edu_class_devices(edu_class_devices)

    edu_class.reload
    return { :edu_class => { :updated => [ edu_class.get_attributes ] } }
  end # self.update
