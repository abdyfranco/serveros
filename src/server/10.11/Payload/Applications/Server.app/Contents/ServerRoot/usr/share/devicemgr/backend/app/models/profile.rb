...
#-------------------------------------------------------------------------
  ...
  def generate_profile_cache
    ...
    if num_ks && num_ks[0] > 0
      ...
      self.profile_cache = (cache_data && pm.substitution_keys == 0 ? Base64.encode64(cache_data) : cache_data)
      self.substitution_keys = pm.substitution_keys || 0
      self.admin_session_id = nil if self.profile_cache
    else
  ...
  def modify_attributes(attr_hash, extended = false)
    ...
    attr_hash.delete('profile_cache')      # Admin doesn't need any of the following
