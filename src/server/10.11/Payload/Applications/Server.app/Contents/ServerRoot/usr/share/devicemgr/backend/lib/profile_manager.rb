...
#-------------------------------------------------------------------------
  ...
  @@ignore_keys = [ 'knobset_attributes', 'id', 'created_at', 'updated_at', 'is_from_servermgr', 'admin_temp_id', 'temporary_id',
                    ...
                    'settings_name', 'settings_identifier', 'settings_version' ]
  ...
  def self.sign_data(data)
    ...
    if MDMLogger.debugOutput?
      ...
      ms = [Benchmark.ms { signed_data = ScepHelper::get_cms_signed_data(identity, cur_settings.server_org_magic, data) }, 0.01].max
      Rails.logger.info("ProfileManager.sign_data completed in #{ms} ms")
    else
      signed_data = ScepHelper::get_cms_signed_data(identity, cur_settings.server_org_magic, data)
    end
  ...
  def self.generate_scep_profile(device)
    ...
    if MDMLogger.debugOutput?
      ms = [Benchmark.ms { scep_challenge = ScepHelper::get_scep_challenge_for_host(od_master) }, 0.01].max
      Rails.logger.info("generate_scep_profile: get_challenge_for_udid completed in #{ms} ms")
    else
      scep_challenge = ScepHelper::get_scep_challenge_for_host(od_master)
    end
    ...
    return self.sign_data(profile);
  end # self.generate_scep_profile
  ...
  def self.generate_signed_profile2(target, device = nil, user = nil, manual_download = false)
    ...
    profile_plist = profile_hash.to_plist
    return self.sign_data(profile_plist), pm.substitution_keys
  ...
  def generate_profile
    ...
    type = @target.library_item_type
    payload_scope = (type.start_with?('Device') ? 'System' : 'User')  # Can only be 'System' scope for profiles that target a device or device group
    ...
    if identification_knob_set
      ...
      payload_content.each { |payload|
        ...
        if payload['PayloadType'] == 'com.apple.ews.account'
        ...
        if payload['PayloadType'] == 'com.apple.ldap.account'
    ...
    profile_hash['PayloadContent'] = merge_payload_type_duplicates(payload_content)
    return self.substituteDeviceAndUserValues(profile_hash)
