...
$logger.info( "oldIP=" + $oldIP + " newIP=" + $newIP + " oldHost=" + $oldHost + " newHost=" + $newHost  )

print "oldIP=" + $oldIP + " newIP=" + $newIP + " oldHost=" + $oldHost + " newHost=" + $newHost + "\n" if $verbose
if ($oldIP == "127.0.0.1" || $newIP == "127.0.0.1")
	msg = "Error: address 127.0.0.1 is not valid for web service"
	print( msg + "\n" ) if $verbose
	$logger.info(msg) 
	exit 1
end
...
$logger.info("Service Proxy SetConfig")
`#{$ServerInstallPathPrefix}/usr/libexec/serviceproxy/SetConfig.py -w`
$logger.info("Service Proxy SetProxyCert")
`#{$ServerInstallPathPrefix}/usr/libexec/serviceproxy/SetProxyCert.py`

$logger.info("Restarting the Service Proxy")
`#{$ServerInstallPathPrefix}/usr/sbin/serviceproxyctl restart`
msg = "Finished changeip_web"
print( msg + "\n" ) if $verbose
$logger.info(msg) 
