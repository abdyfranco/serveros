...
function _ota_service_transaction_challenge($incoming_request)
{
  ...
  SetCVMulti($usage, ['auto_join_profile_id' => CV($aj_profile, 'id'), 'device_id' => CV($device, 'id'), 'checkin_token' => $checkin_token]);
...
function OTAServiceCommon()   // This is only used for auto-enrollment profiles now
{
  ...
  if (!empty($incoming_request['CHALLENGE'])) {
    $device = PerformInTransaction('_ota_service_transaction_challenge', $incoming_request, kDMPGPriorityLow);
    ...
    $device = PerformInTransaction('_ota_service_transaction_udid', $incoming_request, kDMPGPriorityLow);
