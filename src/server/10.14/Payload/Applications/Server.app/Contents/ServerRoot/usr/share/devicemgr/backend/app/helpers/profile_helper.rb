...
#-------------------------------------------------------------------------
  ...
  def self.add_knob_set(library_item_id, incoming_request)
    ...
    MDMAuditor.profile_updated(MDMUtilities.session, profile, "added knob set #{new_knob_set.class}")
  ...
  def self.create(incoming_request)
    ...
    MDMAuditor.profile_created(MDMUtilities.session, profile)
  ...
  def self.destroy(id)
    ...
    if profile
      MDMAuditor.profile_deleted(MDMUtilities.session, profile)
      owner        = profile.owner
  ...
  def self.remove_knob_set(library_item_id, incoming_request)
    ...
    MDMAuditor.profile_updated(MDMUtilities.session, profile, "removed knob set #{knob_set.class}")
  ...
  def self.update(id, incoming_request)
    ...
    MDMAuditor.profile_updated(MDMUtilities.session, profile)
