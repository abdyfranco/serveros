...
#-------------------------------------------------------------------------
  ...
  def self.begin_refresh; MDMUtilities.send_devicemgrd_request({ :command => 'depStartSync', :forceFull => false, :userTriggered => true }, true); end # Trigger an incremental DEP sync, true = async
  ...
  def self.find_all_ios_with_mac_address(remote_guid)
    owner_assignable_devices = Device.get_all_ios_with_mac_address_device_ids
    owner_assignable_devices.unshift(:device)
    return { :remote => { remote_guid.to_sym => [ owner_assignable_devices ] } }
  end # self.find_all_owner_assignable
  ...
  def self.find_matching_ios_with_mac_address(incoming_request, remote_guid, selected_item_id = nil)
    device_array = Device.get_all_ios_with_mac_address_device_ids(incoming_request['search_string'])
    device_array.unshift(:device)
    return { :remote => { remote_guid.to_sym => [ device_array ] } }
  end # self.find_matching_ios_with_mac_address
  ...
  def self.get_app_license_details(incoming_request);   return { :device => { :retrieved => Device::get_app_license_details_for_multiple_by_id(incoming_request['ids']) } };  end
  def self.get_book_license_details(incoming_request);  return { :device => { :retrieved => Device::get_book_license_details_for_multiple_by_id(incoming_request['ids']) } }; end
  def self.get_complete_details(incoming_request);      return { :device => { :retrieved => Device::get_attributes_for_multiple_by_id(incoming_request['ids'], true) } };     end
  def self.get_details(incoming_request);               return self.get_details_for_multiple(incoming_request);                                                               end
  def self.get_details_for_multiple(incoming_request);  return { :device => { :retrieved => Device::get_attributes_for_multiple_by_id(incoming_request['ids']) } };           end
  ...
  def self.update(device_id, incoming_request)
    ...
    # So to be safe, we will only honor the contents of the a few new keys
    # device.update_from_hash(incoming_request)
    # device.save
