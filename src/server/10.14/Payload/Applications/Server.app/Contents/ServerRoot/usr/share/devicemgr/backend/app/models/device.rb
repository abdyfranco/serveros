...
#  has_one :library_item_metadata
  ...
  @@admin_required_attributes = [ 'DeviceName', 'user_id', 'SerialNumber', 'udid', 'IMEI', 'MEID', 'DeviceID', 'last_checkin_time', 'ProductName',
                                  'profile', 'identifier', 'is_mac', 'is_ios', 'vpp_last_invite_requested', 'vpp_last_invite_delivered', 'airplay_password', 'enrollment_state', 'is_dep_device', 'mdm_target_type']
  ...
  def self.get_all_apple_tv_ids
    sql = <<-SQL
      ...
      WHERE    mdm_target_type = 'tv'
         OR    "ProductName" ILIKE '%AppleTV%'
  ...
  def self.get_all_owner_assignable_device_ids(search_string = nil)
    ...
    sql = <<-SQL
      SELECT  id
      FROM    view_valid_devices  -- Filters out devices that would return false for valid_device?
      WHERE   CASE mdm_target_type
              WHEN 'ios' THEN NOT is_multi_user
              WHEN 'mac' THEN id IN (SELECT device_id FROM owner_lab_sessions)
              ELSE TRUE
              END
  ...
  def self.get_all_ios_with_mac_address_device_ids(search_string = nil)
    search = ''
    if search_string
      search = self.connection.quote_string(search_string.gsub(/([_%\|\\])/, '|\1'))    # Escape the special characters in the search string
      Rails.logger.info("Escaped '#{search_string}' to '#{search}'") if MDMLogger.debugOutput?(2)
    ...
    sql = <<-SQL
      ...
      WHERE   mdm_target_type = 'ios'
        AND   "WiFiMAC" IS NOT NULL
        AND   (   "DeviceName"   ILIKE '%#{search}%' ESCAPE '|'
               OR "SerialNumber" ILIKE '%#{search}%' ESCAPE '|')
      ORDER BY LOWER(order_name) ASC NULLS LAST, order_name DESC NULLS LAST, id ASC
    SQL
    return self.connection.select_integer_values_by_index(sql)
  end # self.get_all_ios_with_mac_address_device_ids
  ...
  def self.get_app_license_details_for_multiple_by_id(ids)
    return ids.collect { |id|
      id = Integer(id)
      { :id => id, :app_data => (self.connection.select_json_value_by_index("SELECT dm_app_info_for_device(#{id}, TRUE)") || []) }
    }
  ...
  def self.get_book_license_details_for_multiple_by_id(ids)
    return ids.collect { |id|
      id = Integer(id)
      { :id => id, :book_data => (self.connection.select_json_value_by_index("SELECT dm_media_info_for_device(#{id})") || []) }
    }
  ...
  def get_app_data;                   return (self.connection.select_json_value_by_index("SELECT dm_app_info_for_device(  #{self.id}, FALSE)") || []);                          end
  def get_atv_password_info;          return self.connection.select_json_value_by_index("SELECT dm_get_atv_password_profile_info(#{self.id})");                                 end
  ...
  def get_vpp_user_id
    vpp_user_id = nil
    its_hash = nil
    if self.mac? || self.is_multi_user?
      sql = <<-SQL
        ...
        WHERE    id IN (SELECT   id
                        ...
                        ORDER BY last_checkin_time DESC NULLS LAST
                        LIMIT    1)
      SQL
      its_hash = self.connection.select_string_values_by_index(sql)[0]
    end

    # Fallback to device lim if LS doesn't have the keys set
    unless its_hash
      metadata = self.library_item_metadata
    ...
    if its_hash
      ...
      sql = <<-SQL
        SELECT   id
        FROM     users
        WHERE    od_vpp_its_id_hash  = '#{its_hash}'
           OR    asm_vpp_its_id_hash = '#{its_hash}'
  ...
  def atv?;           return (self.mdm_target_type == 'tv');                        end
  def is_atv?;        return (self.mdm_target_type == 'tv');                        end
  def ios?;           return (self.mdm_target_type == 'ios');                       end
  def is_ios?;        return (self.mdm_target_type == 'ios');                       end
  def is_multi_user?; return (self.mdm_target_type == 'ios' && self.is_multi_user); end
  ...
  def modify_attributes(attr_hash, extended = false)
    ...
    attr_hash['is_ios']            = (attr_hash['mdm_target_type'] == 'ios')
    ...
    if extended
      ...
      attr_hash['has_FDE_PersonalRecoveryKeyCMS'] = ext_attr['SecurityInfo'] && !ext_attr['SecurityInfo'].delete('FDE_PersonalRecoveryKeyCMS').nil?
      ...
      if attr_hash['mdm_target_type'] == 'tv'
       atv_info = self.get_atv_password_info
       attr_hash['airplay_password_set_from'] = atv_info if atv_info
      end
    end
