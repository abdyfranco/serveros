...
def VersionInt(c1, c2=0, c3=0); return (c1*10000+c2*100+c3); end
...
#-------------------------------------------------------------------------
  ...
  def self.included(base)
    ...
    base.class_eval do
      ...
      unless self.method_defined?(:get_all_device_ids_for_task)  # Don't override any existing implementation
        def get_all_device_ids_for_task(task_type)
          ...
          when 'ClearRestrictionsPassword'
            where = "d.mdm_target_type = 'ios' AND d.\"IsSupervised\" AND d.os_version_int >= #{VersionInt(8,0)}"         # iOS 8+

          # os x 10.10 and greater, ios 8.0 and greater, supervised only
          when 'SetDeviceName'
            ios_where  = "d.mdm_target_type = 'ios' AND d.\"IsSupervised\" AND d.os_version_int >= #{VersionInt(8,0)}"    # iOS 8+
            tvos_where = "d.mdm_target_type = 'tv'  AND d.\"IsSupervised\" AND d.os_version_int >= #{VersionInt(10,2)}"   # tvOS 10.2+
            osx_where  = "d.mdm_target_type = 'mac' AND d.os_version_int >= #{VersionInt(10,10)}"                         # macOS 10.10+
            where = "(#{ios_where}) OR (#{osx_where}) OR (#{tvos_where})"

          when 'PushOSXEnterpriseApplications'
            where = "d.mdm_target_type = 'mac' AND d.os_version_int >= #{VersionInt(10,10)}"  # macOS 10.10+
          ...
          when 'EnableLostMode'
            join  = 'library_item_metadata AS m USING (id)'   # JOIN to library_item_metadata to access dynamic_attributes
            where = "d.mdm_target_type = 'ios' AND d.\"IsSupervised\" AND d.os_version_int >= #{VersionInt(9,3)} AND (m.dynamic_attributes->>'IsMDMLostModeEnabled') IS DISTINCT FROM 'true'" # iOS 9.3+
          ...
          when 'DeviceLocation', 'DisableLostMode'
            join  = 'library_item_metadata AS m USING (id)'   # JOIN to library_item_metadata to access dynamic_attributes
            where = "d.mdm_target_type = 'ios' AND d.os_version_int >= #{VersionInt(9,3)} AND (m.dynamic_attributes->>'IsMDMLostModeEnabled') = 'true'"                           # iOS 9.3+

          when 'PlayLostModeSound'
            join  = 'library_item_metadata AS m USING (id)'   # JOIN to library_item_metadata to access dynamic_attributes
            where = "d.mdm_target_type = 'ios' AND d.\"IsSupervised\" AND  d.os_version_int >= #{VersionInt(10,3)} AND (m.dynamic_attributes->>'IsMDMLostModeEnabled') = 'true'"  # iOS 10.3+
          ...
          when 'EnableAppAnalytics'
            join  = 'library_item_metadata AS m USING (id)'   # JOIN to library_item_metadata to access dynamic_attributes
            where = "d.is_multi_user AND d.os_version_int >= #{VersionInt(9,3,2)} AND (m.dynamic_attributes->>'DiagnosticSubmissionEnabled') = 'true' AND (m.dynamic_attributes->>'AppAnalyticsEnabled') IS DISTINCT FROM 'true'" # iOS 9.3.2+

          when 'DisableAppAnalytics'
            join  = 'library_item_metadata AS m USING (id)'   # JOIN to library_item_metadata to access dynamic_attributes
            where = "d.is_multi_user AND d.os_version_int >= #{VersionInt(9,3,2)} AND (m.dynamic_attributes->>'DiagnosticSubmissionEnabled') = 'true' AND (m.dynamic_attributes->>'AppAnalyticsEnabled') = 'true'"                # iOS 9.3.2+

          when 'PasscodeLockGracePeriod'
            where = "d.is_multi_user AND d.os_version_int >= #{VersionInt(9,3,2)}"                                        # iOS 9.3.2+

          when 'EraseDevice'
            where = "d.mdm_target_type <> 'tv' OR d.os_version_int >= #{VersionInt(10,2)}"                                # tvOS 10.2+

          when 'RestartDevice'
            ios_where   = "d.mdm_target_type = 'ios' AND d.\"IsSupervised\" AND d.os_version_int >= #{VersionInt(10,3)}"  # iOS 10.3+
            tvos_where  = "d.mdm_target_type = 'tv'  AND d.\"IsSupervised\" AND d.os_version_int >= #{VersionInt(10,2)}"  # tvOS 10.2+
            macos_where = "d.mdm_target_type = 'mac' AND d.os_version_int >= #{VersionInt(10,13)}"                        # macOS 10.13+
            where = "(#{ios_where}) OR (#{tvos_where}) OR (#{macos_where})"

          when 'ShutDownDevice'
            ios_where   = "d.mdm_target_type = 'ios' AND d.\"IsSupervised\" AND d.os_version_int >= #{VersionInt(10,3)}"  # iOS 10.3+
            macos_where = "d.mdm_target_type = 'mac' AND d.os_version_int >= #{VersionInt(10,13)}"                        # macOS 10.13+
            where = "(#{ios_where}) OR (#{macos_where})"

          when 'UnlockUserAccount', 'SetFirmwarePassword', 'VerifyFirmwarePassword'
            where = "d.mdm_target_type = 'mac' AND d.os_version_int >= #{VersionInt(10,13)}"                              # macOS 10.13+

          when 'EnableBluetooth', 'DisableBluetooth'
            ios_where = "d.mdm_target_type = 'ios' AND d.\"IsSupervised\" AND  d.os_version_int >= #{VersionInt(11,3)}"   # iOS 11.3+
            macos_where = "d.mdm_target_type = 'mac' AND d.os_version_int >= #{VersionInt(10,13,4)}"                      # macOS 10.13.4+
            where = "(#{ios_where}) OR (#{macos_where})"
          end # case task_type
      ...
      unless self.method_defined?(:get_devices_local_users_info)  # Don't override any existing implementation
        def get_devices_local_users_info; return (self.connection.select_json_value_by_index("SELECT dm_get_devices_local_users_info(#{self.id})") || []); end
      end # unless self.method_defined?
      ...
      unless self.method_defined?(:get_os_updates_info)  # Don't override any existing implementation
        def get_os_updates_info
          ...
          info.each { |i|
            ...
            i['Title'] = (names[locale] || names['en'] || names['DEFAULT'] || names.values[0]) unless names.empty?
            i['Title'] ||= 'Missing Update Name'
            names = i['Build']
            i['Title'] += " (#{names})" unless names.empty?
          } # info.each
          return info
        end
      end # unless self.method_defined?
