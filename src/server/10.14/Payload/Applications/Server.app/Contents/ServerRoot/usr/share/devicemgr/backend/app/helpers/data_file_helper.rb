...
#-------------------------------------------------------------------------
  ...
  def self.create_enterprise_app_from_file(data_file_id, skip_push = false)
    ...
    @@final_asset = @@tmp_extract_dir = @@tmp_icon_path = is_mac_app = is_ios_app = is_tv_app = nil
    ...
    is_tv_app  = info_plist['CFBundleSupportedPlatforms'].include? 'AppleTVOS' unless info_plist['CFBundleSupportedPlatforms'].nil?
    ...
    if is_mac_app && file_type == '.app'
      ...
      raise "Failed to compute MD5 hash on generated package file #{pkg_path}" unless status == 0
      @@pkg_hash_md5 = stdout.strip
      stdout, status = MDMUtilities::do_backtick(SHASUM_PATH, '-a', '256', pkg_path)
      raise "Failed to compute SHA256 hash on generated package file #{pkg_path}" unless status == 0
      @@pkg_hash_sha256 = stdout.strip.sub(/^([0-9A-Fa-f]++).*/, '\1')
    ...
    enterprise_app.supported_devices = (is_tv_app ? 16 : (is_mac_app ? 8 : 7))
    enterprise_app.save
  ...
  def self.parse_cert(data_file_id)
    ...
    if file_ext == '.p12' || file_ext == '.pfx'
      ...
      cert_data = { :is_identity    => false,
                    :is_der_encoded => file_data.to_s[0..9] != '-----BEGIN',    # PEM encoded certs begin with '-----BEGIN CERTIFICATE-----'
                    :is_root        => cert.subject.eql?(cert.issuer),
                    :certificate    => { :issuer              => cert.issuer.to_s,
                                         :not_before          => cert.not_before,
                                         :not_after           => cert.not_after,
  ...
  def self._create_app_package(path_to_app)
    ...
    # ident_name = DMHelper::get_mdm_server_identity_name
  ...
  def self._finalize_asset_on_commit
    ...
    if @@pkg_hash_md5
      ...
      begin
        ...
        File.open(path, 'wb') { |f| f.write(@@pkg_hash_md5) }
    ...
    if @@pkg_hash_sha256
      begin
        path = final_asset_path+'.sha256'
        File.open(path, 'wb') { |f| f.write(@@pkg_hash_sha256) }
        FileUtils.chmod(0644, path)
      rescue Exception => e
        Rails.logger.warn("Failed to write SHA256 checksum to '#{path}' (#{e.message})")
      end
    end
