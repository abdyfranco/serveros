...
#-------------------------------------------------------------------------
  ...
  def self.push_applications_to_library_item(apps, target_class, target_id, tethered = false)
    # Sanitize input
    ...
    self.connection.execute("SELECT dm_push_media_to_library_item(#{target_id}, #{app_ids}, #{tethered === true ? 'TRUE' : 'FALSE'})")
  ...
  def self.update_app_assignments(library_item, new_app_infos)
    ...
    app_ids = assign_app_ids.map { |id|
      ...
      i_mode = info['installation_mode']
      a_mode = (a_mode == 'device' ? VPPAppDeviceSNBit : (a_mode == 'maid' ? VPPAppMAIDBit : VPPAppAppleIDBit))
      t_mode = (i_mode.include?('tethered')     ? 'TRUE' : 'FALSE')
      i_mode = (i_mode.start_with?('automatic') ? 'TRUE' : 'FALSE')
      "(#{id},#{li_id},#{a_mode},#{i_mode},#{t_mode})"
    }
    self.connection.execute("INSERT INTO library_items_assets (asset_id, library_item_id, association_type, auto_push, require_tether) VALUES #{app_ids.join(',')}") unless app_ids.empty?
    ...
    existing_app_ids.each { |id|
      ...
      t_mode = (i_mode.include?('tethered')     ? 'TRUE' : 'FALSE')
      i_mode = (i_mode.start_with?('automatic') ? 'TRUE' : 'FALSE')
      self.connection.execute("UPDATE library_items_assets SET association_type = #{a_mode}, auto_push = #{i_mode}, require_tether = #{t_mode} WHERE asset_id = #{id} AND library_item_id = #{li_id}")
      changed = true
