...
window.addEventListener('click', function(e) {
  ...
  if (e.target.className.indexOf('performTask') > -1) {
    ...
    if (!targetObject.closest('.iOSWipeDeviceOptions').length && taskType === 'EraseDevice') {

      var deviceContainer = targetObject.parent().parent(),
        preserveData = deviceContainer.find('.iOSWipeDeviceOptions').length > 0;
      if (preserveData) {

        deviceContainer.find('.iOSWipeDeviceOptions').show();

        $('html,body').animate({
          scrollTop: deviceContainer.find('.iOSWipeDeviceOptions input[name="preserveDataPlan"]').offset().top - 32
        }, 1);
    ...
    if (window.confirm(window.localized.portal_are_you_sure_you_want_to_perform_the + e.target.innerHTML + window.localized.portal_task_on_this_device)) {
      ...
      if ($(e.target)
        .closest('.iOSWipeDeviceOptions')
        .length) {

        performTaskFormInfo.PreserveDataPlan = !!$(e.target)
          .closest('.iOSWipeDeviceOptions')
          .find('input[name="preserveDataPlan"]')
          .val();

        performTaskFormInfo.DisallowProximitySetup = !!$(e.target)
          .closest('.iOSWipeDeviceOptions')
          .find('input[name="disallowProximitySetup"]')
          .val();
      }
