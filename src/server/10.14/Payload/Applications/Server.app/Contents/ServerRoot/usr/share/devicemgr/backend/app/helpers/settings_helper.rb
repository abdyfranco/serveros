...
#-------------------------------------------------------------------------
  ...
  def self.update(settings_id, incoming_request)
    ...
    if incoming_request.key?('admin_prefs')
      user = User.logged_in_user(:refresh => false)   # User must already be logged into the admin , so they will be in the DB
  ...
  def self.verify_apns; return Settings.get_settings.apns_active; end
