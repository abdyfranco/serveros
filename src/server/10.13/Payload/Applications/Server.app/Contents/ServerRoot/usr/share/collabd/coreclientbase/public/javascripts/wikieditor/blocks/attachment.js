...
CC.WikiEditor.AttachmentBlockView = Class.create(CC.WikiEditor.FileBlockView, {
	...
	renderAsHTML: function() {
		...
		var html = "<div class=\"attachment%@\">".fmt((isQuickLookable ? " quicklookable" : "")) +
			"<div id=\"%@\" role=\"button\" aria-label=\"%@\" class=\"right-cap\"%@></div>".fmt(eventDelegateIdentifer + "-right-cap", filename + "_Editor.Block.Attachment.Quicklook".loc(), (isQuickLookable ? " title=\"%@\"".fmt("_Editor.Block.Attachment.QuickLook.Label".loc()) : "")) +
	...
	registerEventHandlers: function() {
		...
		var eventDelegateIdentifer = this.getEventDelegateIdentifer();
		var clickEvent = browser().isMobileSafari() ? 'mousedown' : 'click';
		...
		if (elem.down('.attachment').hasClassName('quicklookable')) {
			globalEventDelegate().registerDomResponderForEventByIdentifer(clickEvent, eventDelegateIdentifer + "-right-cap", this.handleQuickLookFileClicked);
			// This hack is required to trigger the mousedown event on iPads.
