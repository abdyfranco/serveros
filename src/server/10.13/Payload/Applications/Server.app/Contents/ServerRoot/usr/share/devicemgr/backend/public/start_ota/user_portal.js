...
var UserPortal = {
  ...
  updateProfiles: function(profilesInfo) {
    ...
    for (i = 0, l = profileNodes.length; i < l; i++) {
      var n = profileNodes[i],
        profileId = parseInt(n.id.match(/.*-(.*)-.*/)[1], 10);
  ...
  getProfileNameForType: function(name, type) {
    var rv = "";
    switch (type) {
      case "ios":
        rv = localized.portal_settings_name_type_ios.replace("%1$s", name);
        break;
      case "mac":
        rv = localized.portal_settings_name_type_mac.replace("%1$s", name);
        break;
      case "tv":
        rv = localized.portal_settings_name_type_tv.replace("%1$s", name);
        break;
    }
  ...
  renderProfile: function(profile) {
    ...
    for (var j = 0; j < targetTypes.length; j++) {
      var type = targetTypes[j],
        targetKnobSets = profile.knob_set_types[type];

      if (targetKnobSets && targetKnobSets.length > 0) {

        var profileId = profile.id + "-" + type,
          profileDownloadUrl = PM_WEBAPP_URI_ROOT + '/profile/get_a_la_carte_profile/' + profile.id + '?target_type=' + type,
          knobSetTypesHTML = "";

        // Remove the no profiles available message if it's there
        $('#no_profile_available').remove();

        // Remove the updated profile from the list if it already exists
        $('#profile-' + profileId).remove();

        for (var i = 0, l = targetKnobSets.length; i < l; i++) {
          var knobSetType = targetKnobSets[i];
        ...
        profileHTML = '<div class="profile" id="profile-' + profileId + '">' +
          '<div class="name">' + t.escapeHtml(this.getProfileNameForType(profile.name, type)) + '</div>' +
          ...
          '<div class="button"><a title="' + localized.portal_download_and_install_profile + '" href="' + profileDownloadUrl + '"><canvas></canvas></a></div>' +
  ...
  renderEnrollmentUI: function() {
    ...
    enrollmentUIHtml = '<div id="addDeviceFakeDevice" class="device">' +
      ...
      addDeviceHelperHtml +
      '</div>' +
      '<form id="addDeviceForm" action="' + PM_MDM_URI_ROOT + '/mdm_enroll"  method="post">' +
  ...
  getTasksHtml: function(device) {
    ...
    if (device.is_ios) {
      tasksHtml += '<div class="iOSWipeDeviceOptions" style="display:none;">' +
        '<form>' +
        '<div><input type="checkbox" name="preserveDataPlan"/>' +
        '<label>' + window.localized.portal_preserve_data_plan + '</label></div>' +
        '<div><input type="checkbox" name="disallowProximitySetup"/>' +
        '<label>' + window.localized.portal_disallow_proximity_setup + '</label><div>' +
        '<div class="tasks"><a href="javascript:void(0);" class="performTask EraseDevice">' + window.localized.portal_wipe_task + '</a></div>' +
        '</form>' +
        '</div>';
    }
