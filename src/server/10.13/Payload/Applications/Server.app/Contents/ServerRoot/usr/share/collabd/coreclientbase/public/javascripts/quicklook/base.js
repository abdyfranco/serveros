...
CC.QuickLook.GlobalQuickLookInstance.prototype = {
	...
	mDefaultAudioPanelSize: {'width': 264, 'height': 315},
	...
	showPreviewPanelAnimationDidFinish: function(inEvent) {
		// Make sure we don't trigger the quiclook multiple times (opacity and transform)
		// Trigger only on 'transform' or '-webkit-transform' event type
		if ((inEvent.propertyName == "transform") || (inEvent.propertyName == "-webkit-transform")) {
			if (this.mPreviewInfo && this.mPreviewInfo.isQuickLookable) {
				return this.configurePreviewPanel(this.mPreviewInfo);
			}			
	...
	hidePreviewPanel: function() {
		// Stop playing Audio or Video
		if (this.mPreviewInfo) {
			if (this.mPreviewInfo.mediaType == 'audio') {
				$('quicklook_panel').down('audio').pause();
			}
			else if (this.mPreviewInfo.mediaType == 'movie') {
				$('quicklook_panel').down('video').pause();
			}			
		}
