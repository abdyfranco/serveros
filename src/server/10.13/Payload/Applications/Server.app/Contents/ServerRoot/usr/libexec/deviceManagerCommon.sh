...
# Configure database access privileges
cp -f $DEVICEMGR_CONFIG_PATH/pg_*.conf   $PG_DATA_DIR/
chown $PM_USERNAME:$PM_GROUPNAME $PG_DATA_DIR/pg_*.conf
chmod 640 $PG_DATA_DIR/pg_*.conf
...
sudo -u $PM_USERNAME $SERVER_ROOT/usr/bin/xpostgres -a $PM_PG_CONFIG_FILE &
pg_pid=$!
log "Started xpostgres with PID $pg_pid"

# We now have to connect to get postgres to actually start...
$DMDB -l

wait_for_postgres $pg_pid            # This will wait forever for postgres to start, but if xpostgres dies we'll catch it in wait_for_postgres and abort this script
...
# Load dm_helper before migration because we might need it
