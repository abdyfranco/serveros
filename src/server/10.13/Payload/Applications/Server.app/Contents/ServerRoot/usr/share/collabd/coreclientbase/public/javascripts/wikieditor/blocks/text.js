...
CC.WikiEditor.TextBlockView = Class.create(CC.WikiEditor.BlockView, CC.WikiEditor.Mixins.Prediction, CC.WikiEditor.Mixins.Comparable, {
	...
	insertHTML: function(inHTML) {
		if (browser().isIE()) {
			if (document.selection.type == 'None') {
				this._editable.innerHTML += inHTML;
			}
			else {
				var range = document.selection.createRange().pasteHTMLForIE(inHTML);				
	...
	addLink: function(inURL, inLinkText, inTarget) {
		...
		if (inURL && inURL != "" && !inURL.isWhitespace()) {
			...
			if (globalEditorTextSelectionDelegate().currentSelectionIsEmpty()) {
				...
				if(inURL.indexOf('?') > -1) { // check if there's already a an '?' in the url
	...
	formatAllLinks: function(inOptElement) {
		...
		for (linkIdx = 0; linkIdx < links.length; linkIdx++) {			
			...
			// Define the target attribute value based on existing values or set it to the default value
			
			if (href) {
				if (href.indexOf("target=_blank") > -1) { 
					href = href.replace("_blank", "_new"); // replace blank by new to avoid openning 2 tabs (bug in safari...)
					target = "_new";
				}
				else if (href.indexOf("target=_self") > -1) { 
					target = "_self";
				}									
				else if (target == "_blank") { // Don't forget to replace the target attribute from blank to new as well
					target = "_new";		
				}									
				// if we're in a case where target is empty and there's no target attribute already in the href
				// then set target=_self by default (e.g. migration case from older Server.app)
				else if (!target) {
					target = "_self";								
					var cleanedHref = this.cleanUpHref(href);
					// check if there's already a an '?' in the href
					if(href.indexOf('?') > -1) {								
						dataEditableLink = cleanedHref + '?target=' + target;
					} else {
						dataEditableLink = cleanedHref + '&target=' + target;
					}
				}	
				...
				link.setAttribute('href', this.cleanUpHref(dataEditableLink));
				link.setAttribute('target', target);
				link.setAttribute('data-editable-link', dataEditableLink);
			}
		}		
	},
	// clean up the href from existing target attribute
	cleanUpHref: function(dirtyHref) {
		var cleanHref = dirtyHref;
		cleanHref = cleanHref.replace("&target=_self", "");
		cleanHref = cleanHref.replace("&target=_blank", "");
		cleanHref = cleanHref.replace("&target=_new", "");
		cleanHref = cleanHref.replace("&target=", "");	
		cleanHref = cleanHref.replace("?target=_self", "");
		cleanHref = cleanHref.replace("?target=_blank", "");
		cleanHref = cleanHref.replace("?target=_new", "");
		cleanHref = cleanHref.replace("?target=", "");			
