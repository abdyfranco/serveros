...
#-------------------------------------------------------------------------
  ...
  OSVersion11_3 = 110300
  OSVersion10_13_4 = 101304
      ...
      unless self.method_defined?(:get_all_device_ids_for_task)  # Don't override any existing implementation
        def get_all_device_ids_for_task(task_type)
          ...
          when 'RestartDevice'
            ...
            macos_where = "d.mdm_target_type = 'mac' AND d.\"OSVersion\" ~* '^([1-9][0-9]\\.(1[3-9]|[2-9][0-9])|1[1-9]|[2-9][0-9])'"      # macOS 10.13+
          ...
          when 'UnlockUserAccount', 'SetFirmwarePassword', 'VerifyFirmwarePassword'
            where = "d.mdm_target_type = 'mac' AND d.\"OSVersion\" ~* '^([1-9][0-9]\\.(1[3-9]|[2-9][0-9])|1[1-9]|[2-9][0-9])'"  # macOS 10.13+

          when 'EnableBluetooth', 'DisableBluetooth'
            ios_where = "d.mdm_target_type = 'ios' AND d.\"IsSupervised\" AND  d.os_version_int >= #{OSVersion11_3}"  # iOS 11.3+
            macos_where = "d.mdm_target_type = 'mac' AND d.os_version_int >= #{OSVersion10_13_4}"                     # macOS 10.13.4+
            where = "(#{ios_where}) OR (#{macos_where})"
          end # case task_type
