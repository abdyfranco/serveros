...
#-------------------------------------------------------------------------
  ...
  def logout_admin
    ...
    render :json => { :redirect_url => "#{url_prefix}/auth/logout?redirect=#{url_prefix}/profilemanager" }, :status => 403 # Not 30x as this is now an AJAX call
  end # logout_admin
  ...
  def logout_user
    session[:user] = nil
    url_prefix = request_url_host
    redirect_to_full_url("#{url_prefix}/auth/logout?redirect=#{url_prefix}/mydevices/", 302)
  end # logout_user
  ...
  def verify_auth_token_common
    ...
    if File.exist?('/Library/Server/Wiki/Config/shared/shared_secret')
      ...
      when 32
        hash = OpenSSL::HMAC.digest(OpenSSL::Digest::Digest.new('sha256'), secret, verifyBody)
      when 20
