...
function dateToColummNumber(inDate, inStartWeekDayPref) {
    // If inDate is Sunday and inStartWeekDayPref is Monday, ensures result is 6, not -1.
					...
					if (Element.firstChildWithNodeName(resourceType, 'schedule-inbox')) {
						...
						if (supported) {
							Element.childrenWithNodeName(supported, 'comp').each(function(compNode) {
								if (compNode.getAttribute('name') == 'VEVENT') {
									supportsEvents = true;
								}
							}.bind(this));
						} else
...
var ExpandedCalendarEvent = Class.create(CalendarEvent, {
	...
	deleteFromServer: function($super, inCallback) {
		...
		if (this.recurrenceChangeAction() == CalendarEvent.RecurrenceChangeAction.AllFuture) {
			// Attendee event has to be entirely deleted
			if (!this.organizerIsPrincipal()) {
				return this.mParentEvent.mParentCalendarFile.deleteEvent(inCallback);
			} else {
...
SettingsDialogManager.prototype = {
	initialize: function() {
		...
		var dialogFocusCallback = function() {
			var gotTimezonesCallback = function(inTimezones) {
				// making sure the select input is set on the most recent timezone selected by the user
				var currentSelectedTimezone = timezoneService().selectedTimezone();
				...
				inTimezones.each(function(tzid, i) {
					...
					if (tzid == currentSelectedTimezone) selectIndex = i;
				});
	...
	show: function() {
		...
		// manual focus, for some reason focus() isn't triggered automatically
		dialogManager().focus();
...
CalendarViewController.prototype = {
			...
			var gotCalendarCollection = function() {
				...
				var gotAccountsUserCanAccess = function(inAccountsToDelegate) {
					...
					if (inAccountsToDelegate.length > 0) {
						if ($('calendar_title')) {
							// Remove existing delegate menu
							if ($('calendar_title').down('span').down('span')) {
							    $('calendar_title').down('span').removeChild($('calendar_title').down('span').down('span'));
							}
							if ($('calendar_title').down('span').down('select')) {
							    $('calendar_title').down('span').removeChild($('calendar_title').down('span').down('select'));
							}
	...
	updateAppointmentEntry: function(inAppointment) {
		var movedCallback = function() {
			dialogManager().hideProgressMessage();
			// make sure we remember the appointment and transfer over existing display elements
			if (this.mAppointments[inAppointment.uid]) {
				inAppointment.displayNodes = this.mAppointments[inAppointment.uid].displayNodes
...
EventRecurrenceDialogManager.prototype = {
	...
	showDeleteFirstDialogForAppointment: function(inAppointment) {
		...
		if (!$('appointment_recur_warn_delete_first_dialog')) {
			...
			insertAfter(Builder.node('input', {
				...
				value: '_Calendar.Dialogs.DeleteFirst.DeleteAll'.loc()
	...
	showDeleteNthDialogForAppointment: function(inAppointment) {
		if (!$('appointment_recur_warn_delete_nth_dialog')) {
			var dlg_title = '_Calendar.Dialogs.DeleteNth.Description'.loc();
			var btn_value = '_Calendar.Dialogs.DeleteNth.DeleteAllFuture'.loc();
			if (!inAppointment.organizerIsPrincipal()) {
				dlg_title = '_Calendar.Dialogs.DeleteNth.DescriptionAll'.loc();
				btn_value = '_Calendar.Dialogs.DeleteNth.DeleteAll'.loc();
			}
