...
module DeviceGroupHelper
  ...
  def self.create(incoming_request)
    ...
    return { :device_group => { :created => [ device_group.get_attributes ] } }
  end
  ...
  def self.get_complete_details(incoming_request)
    data = DeviceGroup::get_attributes_for_multiple_by_id(incoming_request["ids"], true)
    return { :device_group => { :retrieved => data } }
  end
  ...
  def self.get_details_for_multiple(incoming_request)
    data = DeviceGroup::get_attributes_for_multiple_by_id(incoming_request["ids"])
    return { :device_group => { :retrieved => data } }
  end
  ...
  def self.get_library_item_tasks(item_id)
    task_array = LibraryItemTask.get_all_tasks_details("DeviceGroup", item_id)
  ...
  def self.update(device_group_id, incoming_request)
    ...
    device_group.reload
    return { :device_group => { :updated => [ device_group.get_attributes ] } }
  end
