  ...
  def self.included(base)
      ...
      if self.method_defined?(:table_name)  # Skip any classes that have their own specific implementation or that don't have a table_name class method
        ...
        if !self.method_defined?(:get_attributes_for_multiple_by_id)
          def get_attributes_for_multiple_by_id(ids, extended = false)
            ...
            data = objs.collect { |u| u.get_attributes(extended) }
            return data
          end
        end
    ...
    base.class_eval do
      ...
      def update_from_hash(hash)
        ...
        hash['admin_temp_id'] = (ses && ses != '' && tid && tid != '' ? "#{ses},#{tid}" : nil)
        ...
        hash.each_pair { |key, val|
          ...
          if self.respond_to?(setter) || self.is_a?(MDMDynamicAttributes)
            ...
            next if a && ((m = a.macro) == :has_many || m == :has_and_belongs_to_many) # This method does not set to-many relationships

            Rails.logger.error("#{setter}#{val}") if MDMLogger.debugOutput?(3)
      ...
      def update_model_to_many_relationships(incoming_request)
        ...
        self.class.reflect_on_all_associations.each { |association|
          ...
          next if self.respond_to?(:also_modifies) && !self.also_modifies.include?(rel_type)
          ...
          sql = <<-SQL
            WITH del AS (
              ...
              RETURNING "#{rel_id_col}"
            )
            UPDATE "#{rel_table_name}"                                -- ...and touch the updated_at on the related rows
            SET    updated_at = DEFAULT
            WHERE  id IN (SELECT "#{rel_id_col}" FROM del)
          SQL
          ...
          sql = <<-SQL
            WITH ins AS (
              ...
              LEFT OUTER JOIN "#{join_table_name}" AS j
                           ON j."#{rel_id_col}" = r.id AND j."#{self_id_col}" = #{my_id}
              WHERE           r.id IN (#{ids})
            ...
            UPDATE "#{rel_table_name}"                                -- ...and touch the updated_at on the related rows
            SET    updated_at = DEFAULT
            WHERE  id IN (SELECT "#{rel_id_col}" FROM ins)
          SQL
          self.connection.execute(sql)
        }
