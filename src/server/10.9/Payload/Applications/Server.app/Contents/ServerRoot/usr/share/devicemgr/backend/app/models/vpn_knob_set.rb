...
class VpnKnobSet < KnobSet
  ...
  def IPSec
    ipSec = self['IPSec']
    ipSec["SharedSecret"] = Base64.decode64(ipSec["SharedSecret"]) if ipSec && ipSec["SharedSecret"] && ipSec["SharedSecret"].class == BinaryData
    return ipSec
  end
  ...
  def modify_payload_hash(payload_hash)
    ...
    if payload_hash["VPN"]
      ...
      if type == 'L2TP' || type == 'PPTP' || type == 'IPSec'
        ...
        # Remove VPNSubType and VendorConfig
        payload_hash.delete('VPNSubType')
        payload_hash.delete('VendorConfig')
