...
function authenticate_checkin_incoming_request(&$target, $incoming_request)
{
  ...
  if (isset($incoming_request['DigestResponse'])) {
    ...
    if (dmx_validate_digest_response($uuid, $digest_response, $_SERVER['REQUEST_METHOD']) !== FALSE) {
      $hash = base64_encode(hash_hmac('sha1', $uuid, GenerateUUID(), TRUE));
  ...
  $user = FindInDatabase('users', 'id', CV($target, 'user_id'));
...
function _checkin_transaction($args)
{
  ...
  $target = Target_for_checkin_request($incoming_request, TRUE);  // Will only return NULL if MessageType = 'CheckOut'
  ...
  case 'TokenUpdate':
    ...
    $new_token = base64_encode($incoming_request['Token']);
    ...
    if (isset($incoming_request["UnlockToken"]) && !Target_is_lab_session($target)) {
      ...
      SetCVDA($ext, 'unlock_token', base64_encode($incoming_request['UnlockToken']));
      SaveToDatabase($ext);
    }
