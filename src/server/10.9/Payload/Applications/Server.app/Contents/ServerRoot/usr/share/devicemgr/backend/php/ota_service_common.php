...
function GenerateMDMBindingProfile($uuid, $mdm_acl, $checkin_token)
{
  ...
  $raw_root_cert  = dmx_get_scep_root_certificate($od_master);
  ...
  $scep_challenge = dmx_get_challenge_for_udid($od_master, $uuid);
  ...
  if (CHECK_CLIENT_CERTS && $gNewURL) {
    $checkin_url = "$base/devicemanagement/api/device/mdm_checkin";
    $server_url  = "$base/devicemanagement/api/device/mdm_connect";
  } else {
    $checkin_url = "$base/devicemanagement/api/device/checkin";
    $server_url  = "$base/devicemanagement/api/device/connect";
  }
  ...
  $trust = ParseYAML($cur_settings['trust_payloads']);
  if (!empty($trust)) {
    foreach ($trust as $pl) {
...
function GetMDMACLFromUserAgentHeader(&$is_mac = NULL)
{
  ...
  if ($is_mac) {
    if (preg_match("/^.*OS X ([0-9]+)[_\.]([0-9]+).*/", $ua, $matches)) {
      $v1 = $matches[1];
      $v2 = $matches[2];
      LogMsg("OSX version $v1.$v2", 1);
      if ($v1 > 10 || ($v1 == 10 && $v2 >= 9)) $mdm_acl |= 4096; // OS X 10.9 and later
    } else DieInternalError("Unable to identify OSX version from User-Agent header");
  } else {
    if (preg_match("/^.*OS ([0-9]+)[_\.]([0-9]+).*/", $ua, $matches)) {
      $v1 = $matches[1];
      $v2 = $matches[2];
      LogMsg("iOS version $v1.$v2", 1);
      if ($v1 >= 5) $mdm_acl |= 4096;  // iOS 5 and later
...
function _generate_scep_profile($device)
{
  ...
  $scep_url       = getSCEPUrl($scep_host);
  if (empty($scep_url)) DieInternalError("Could not retrieve SCEP server URL.");
  $scep_challenge = dmx_get_challenge_for_udid($od_master, CV($device, 'udid'));
  if (empty($scep_challenge)) DieInternalError("Could not retrieve SCEP challenge.");
...
function _ota_service_transaction_challenge($incoming_request)
{
  ...
  $usage_log = CV($aj_profile, 'usage_log');
  if (empty($usage_log)) $usage_log = [];
  $incoming_request['OUR_OWN_TIMESTAMP'] = gmdate(DateTime::ISO8601);   // Add this so we can display it in the Admin UI
  $usage_log[] = $incoming_request;
  SetCV($aj_profile, 'usage_log', $usage_log);
  SaveToDatabase($aj_profile);
...
function OTAServiceCommon()   // This is only used for auto-enrollment profiles now
{
  ...
  if (!empty($incoming_request['CHALLENGE'])) {
    ...
    $final_output = GenerateMDMBindingProfile(CV($device, 'udid'), CV($device, 'mdm_acl'), CV($device, 'pending_checkin_token')); // Another thing we shouldn't be doing inside of a transaction
  } else {
