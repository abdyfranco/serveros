...
class Array
  ...
  def fix_after_load_from_YAML
    array = []
    self.each { |obj| array.push(obj.respond_to?(:fix_after_load_from_YAML) ? obj.fix_after_load_from_YAML : obj) }
    return array
  end
...
class Hash
  ...
  def fix_after_load_from_YAML
    ...
    self.each_pair { |key, value|
      ...
      if type
        ...
        when 'BOOLEAN'
          value = !(!value || value == 0 || value == '0' || value == 'f' || value == 'F' || value == 'false' || value == 'FALSE')
        else
          raise "Unknown dynamic attribute type #{type}"
        end
      else
        value = value.fix_after_load_from_YAML if value.respond_to?(:fix_after_load_from_YAML)
      end
  ...
  # This method is called separately for all children of containers being serialized, so there's no need to recurse here.
  alias_method :real_to_yaml, :to_yaml
  def to_yaml(options={})
    hash = {}
    self.each_pair { |key, value|
      key = key.to_s
      ...
      if value.kind_of?(Time)
        ...
        hash["._#{key}"] = 'DATA'
      elsif value.kind_of?(FalseClass) || value.kind_of?(TrueClass)
        hash[key]        = (value ? 1 : 0)
        hash["._#{key}"] = 'BOOLEAN'
      else
        hash[key] = value
    ...
    return hash.real_to_yaml(options)
  end
...
module Syck
  class << self
    ...
    alias_method :real_load, :load
    def load(*args)
      obj = real_load(*args)
      obj = obj.fix_after_load_from_YAML if obj && obj.respond_to?(:fix_after_load_from_YAML)
      return obj
    end
