...
#
# If we have the OD CA cert file exported, then we need to install our apache SSL config file. Otherwise we need an empty file here.
#
SSL_DST_CONF_PATH=$SERVER_LIBRARY_PATH/Web/Config/apache2/httpd_devicemanagement_ssl.conf
SSL_SRC_CONF_PATH=$DEVICEMGR_CONFIG_PATH/httpd_devicemanagement_ssl.conf
if [ -e $SSL_DST_CONF_PATH ]; then
  rm -rf $SSL_DST_CONF_PATH     # Always delete the destination, we'll set it correctly immediately below
fi
if [ -s $PM_CONFIG_DIR/SSLCACertificateFile.pem ]; then
  cp $SSL_SRC_CONF_PATH $SSL_DST_CONF_PATH
  /usr/sbin/apachectl graceful
else  
  touch $SSL_DST_CONF_PATH
fi
...
# Make sure we have a symlink to php-fpm
...
/usr/bin/find $PM_LIBRARY_DIR/    \( -type f -o -type d \) -print | /usr/bin/xargs chown $PM_USERNAME:$PM_GROUPNAME
/usr/bin/find $SERVICE_DATA_LINK/ \( -type f -o -type d \) -print | /usr/bin/xargs chown $PM_USERNAME:$PM_GROUPNAME
# chmod -R u=rwX,go=rX $SERVICE_DATA_LINK/ -- but only for files and directories
/usr/bin/find $PM_LIBRARY_DIR/    \( -type f -o -type d \) -print | /usr/bin/xargs chmod u=rwX,go=rX
/usr/bin/find $SERVICE_DATA_LINK/ \( -type f -o -type d \) -print | /usr/bin/xargs chmod u=rwX,go=rX
...
cp -f $SERVER_ROOT/Library/Preferences/com.apple.DeviceManagement.postgres.plist $PM_CONFIG_DIR/PostgreSQL_config.plist
...
function child_died()
{
  ...
  if [ "$pids" -eq 0 ]; then
    ...
    log "Received spurious SIGCHLD signal, ignoring"
  fi
} # child_died
