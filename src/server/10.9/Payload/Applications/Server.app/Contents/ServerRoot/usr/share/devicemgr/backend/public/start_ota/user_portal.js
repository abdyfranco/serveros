...
var UserPortal = {
  ...
  initSelectedTab: function() {
    ...
    if (c.apns_active && c.od_active) {
      ...
      if (lastSelectedTab === 'settingsShown') {
  ...
  updateProfiles: function(profilesInfo) {
    ...
    for (var i = 0, l = profileNodes.length; i < l; i++) {
    ...
    if (updatedProfiles) {
      for (var i = 0, l = updatedProfiles.length; i < l; i++) {
  ...
  renderProfile: function(profile) {
    ...
    if (knob_set_types.length > 0) {
      ...
      '<div class="button"><a title="' + localized.portal_download_and_install_profile + '" href="/devicemanagement/api/profile/get_a_la_carte_profile/' + profile.id + '"><canvas></canvas></a></div>' +
  ...
  currentDeviceName: function() {
    return localized.portal_this + ' ' + this.getDeviceType();
  },
  ...
  renderEnrollmentUI: function() {
    var config = UserPortalConfig,
      ...
      deviceIconUrl = '/devicemanagement/api/device/get_icon_for/' + deviceType.toLowerCase(),
      stupidFiller = '&lt;Generic Device Type&gt;',
    ...
    if (deviceType !== '') {
      ...
      enrollThisDeviceText = enrollThisDeviceText.replace(stupidFiller, deviceType);
      onceEnrolledText = onceEnrolledText.replace(stupidFiller, deviceType);
    }
    ...
    enrollmentUIHtml = '<div id="addDeviceFakeDevice" class="device">' +
      ...
      addDeviceHelperHtml +
      '</div>' +
      '<form id="addDeviceForm" action="/devicemanagement/mdm/mdm_enroll"  method="post">' +
  ...
  getRemoveDeviceHtml: function() {
    return '<div class="removeDevice"><a href="javascript:void(0)" title="Remove Device" class="removeDeviceLink">' +
  ...
  renderDevice: function(device) {
    var currentDevice = (device.identifier === window.localStorage.thisDeviceToken),
      newDeviceHTML = '',
      removeDeviceHTML = UserPortalConfig.allow_enrollment_via_portal ? this.getRemoveDeviceHtml() : '',
    ...
    newDeviceHTML += '<div class="device" id="' + deviceDomId + '">' +
      ...
      '<img src="/devicemanagement/api/device/get_icon_for/' + device.ProductName.toLowerCase().replace(',', '_') + '" />' +
    ...
    if (UserPortalConfig.can_lock_device) {
      newDeviceHTML += '<a href="javascript:void(0)" class="performTask DeviceLock">' + window.localized.portal_lock_task + '</a>';
    }

    if (UserPortalConfig.can_wipe_device) {
      newDeviceHTML += '<a href="javascript:void(0)" class="performTask EraseDevice">' + window.localized.portal_wipe_task + '</a>';
    }

    if (!device.is_mac && UserPortalConfig.can_clear_passcode_on_device) {
      newDeviceHTML += '<a href="javascript:void(0)" class="performTask ClearPasscode">' + window.localized.portal_clear_passcode_task + '</a>';
    }
    ...
    if (device.is_mac) {
      newDeviceHTML += '<div class="macLockWipePin" style="display:none;">' +
        '<form>' +
    ...
    if (device.last_task) {
      newDeviceHTML += this.getTaskHtml(device.last_task);
    }
...
$(document).ready(function() {
  ...
  $('span.task-completed-date').each(function(index, node) {
    var node = $(node),
