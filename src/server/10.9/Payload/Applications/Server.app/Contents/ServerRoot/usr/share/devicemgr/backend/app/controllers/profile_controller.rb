...
class ProfileController < ApplicationController
  ...
  skip_before_filter :verify_admin_logged_in, :only => [:get_a_la_carte_profile, :get_ssl_cert_profile]
  skip_before_filter :verify_post, :only => [:get_signed_profile, :get_raw_profile, :get_a_la_carte_profile, :get_ssl_cert_profile]
  before_filter :verify_apns, :only => [:get_signed_profile]
  before_filter :verify_od_apns, :only => [:run_task]
  before_filter :verify_user_portal_access, :only => [:get_ssl_cert_profile]
  ...
  def get_ssl_cert_profile
    cur_settings = Settings.get_settings
    organization = cur_settings.server_organization

    profile_name = sprintf(I18n.t("trust_profile_display_name"), organization).gsub(/\s+/, '_')+".mobileconfig"
    profile_plist = ProfileManager::generate_trust_profile()
    response.headers['Content-Type'] = 'application/x-apple-aspen-config'
    response.headers['Content-Disposition'] = "attachment; filename=\"#{profile_name}\""
    render :xml => profile_plist
  end
