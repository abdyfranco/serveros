...
class Profile < ActiveRecord::Base
  ...
  def generate_profile_cache
    ...
    Rails.logger.info("Profile#generate_profile_cache = #{(cache_data ? cache_data.to_s : 'NULL')}") if MDMLogger.debugOutput?(2)
    ...
    self.signing_certificate_id   = 0;  # Unsigned, if it needs to be signed, the DB will detect that, block the appropriate tasks and have devicemgrd sign it
  ...
  def set_owner_from_incoming_request(incoming_request)
    ...
    Rails.logger.error("#{self.class}\#set_owner_from_incoming_request(#{incoming_request})")
    OwnerClassToRelation.each { |cls, key|
      attribute = key.to_s + '_id'
      Rails.logger.error("#{self.class}\#set_owner_from_incoming_request: attribute=#{attribute}")
