...
class UnifiedApplication < ActiveRecord::Base
  ...
  def self.push_applications_to_library_item(apps, target_class, target_id)
    ...
    self.connection.execute("SELECT dm_push_vpp_apps_to_library_item(#{app_ids}, '#{target_class}', #{target_id})")
    
    # apps.each { |app_id|
    #     self.connection.execute("SELECT dm_push_vpp_app_to_library_item(#{app_id}, '#{target_class}', #{target_id})")
    # }
  end
  ...
  def before_save
    ...
    # Note that these values may NOT be edited once set, hence the COALESCE on each column being set
    ...
    setable_columns.each { |col|
      ...
      val = "'#{self.connection.quote_string(val)}'" unless col == 'supported_devices' || col == 'size_in_kb'
      set.push "#{col} = #{val}"
    }
    unless set.empty?
      ...
      MDMUtilities.on_rollback { |will_retry|
        unless will_retry
          MDMUtilities.process_in_transaction {
            set.push('updated_at = dm_current_timestamp()')
  ...
  def modify_attributes(attr_hash, extended = false)
    ...
    attr_hash['icon_url'] = "#{Settings.url_base}/devicemanagement/ipa/#{url}" unless url.start_with?('http')
  ...
  def update_app_assignments(new_user_ids, new_group_ids)
    ...
    unless add_ids.empty?
      values = add_ids.collect { |lid| "(#{id},#{lid})" }
      values = values.join(',')
      sql = <<-SQL
        INSERT INTO library_items_assets
                    (asset_id, library_item_id)
