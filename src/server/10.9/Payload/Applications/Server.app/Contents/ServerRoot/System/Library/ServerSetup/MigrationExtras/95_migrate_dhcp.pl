#!/usr/bin/perl -w
#
# Post-install migration script for DHCP
#                                                                                                                                                         
# component: servermgr_dhcp
#
...
my $scriptName="95_migrate_dhcp.pl";
...
. "\t --sourceVersion <ver> The version number of the old system (like 10.7 or 10.6).\n"
...
if ($sourceVersion =~ /10\.[67]/) {
    migrateConfigData;
    migrateCommonData;
}
else {
    print "$scriptName: Migrating DHCP and NetBoot data from $sourceVersion to 10.8 is not supported\n";
}
...
#
# Function to tell Mobile Internet Sharing to leave the DHCP config alone
#

sub fixNatPlist
{
    my $natPlist = "$targetRoot/Library/Preferences/SystemConfiguration/com.apple.nat.plist";
    print "$scriptName: [not implemented] setting flags in $natPlist\n";
    
}

#
# Function to migrate DHCP/NetBoot config 
#
sub migrateConfigData
{
    #
