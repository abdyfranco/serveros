...
#  has_one :library_item_metadata
  ...
  has_and_belongs_to_many :provisioning_profiles      # no optimization as this currently is unused
  ...
  def self.get_all_apple_tv_ids
    sql = <<-SQL
      ...
      ORDER BY "DeviceName" ASC NULLS LAST
    SQL
  ...
  def self.get_all_ids_for_admin(search_string = nil)
    ...
    if search_string
      ...
      where = "WHERE \"DeviceName\" ILIKE '%#{search}%' ESCAPE '|'"
    end
    ...
    sql = <<-SQL
      ...
      FROM    view_valid_devices                        -- Filters out devices that would return false for valid_device?
      #{where}
      ORDER BY "DeviceName" ASC NULLS LAST, id ASC
    SQL
    return self.connection.select_integer_values_by_index(sql)
  end
  ...
  def self.get_all_ids_matching_detailed_for_admin(search_string)
    ...
    sql = <<-SQL
      ...
      UNION
        SELECT  d.id, d."DeviceName"                          -- Union in all the devices owned by users who match the search criteria
  ...
  def enrollment_state
    return (((self.hp_singleton_tasks | self.singleton_task_type) & (1 << 29)) != 0 ? 'unenrollment_pending' : (self.placeholder_device? ? 'placeholder' : 'enrolled'))
  end
  ...
  def get_changed_provisioning_profiles
    # This currently isn't used, as we don't support provisioning profiles
    ext = self.library_item_metadata
    ppl = ext.ProvisioningProfileList if ext
    provisioning_profile_uuids = (ppl ? ppl.collect { |pp_hash| pp_hash["UUID"] } : [])
    changed_provisioning_profiles = self.provisioning_profiles.select { |pp| pp.updated_at > self.updated_at || !provisioning_profile_uuids.include?(pp.UUID) }
    return changed_provisioning_profiles
  end
  ...
  def is_mdm_removable?
    # It IS removable UNLESS we have a dep_profile and the is_mdm_removable is EXPLICITLY set to false
    return !(self.dep_profile && self.dep_profile['is_mdm_removable'] === false)
  end
