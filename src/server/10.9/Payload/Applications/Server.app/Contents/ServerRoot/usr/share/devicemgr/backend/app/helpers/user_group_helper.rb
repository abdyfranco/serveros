...
module UserGroupHelper
  ...
  def self.get_complete_details(incoming_request)
    group_data = UserGroup.get_attributes_for_multiple_by_id(incoming_request["ids"], true)
    return { :user_group => { :retrieved => group_data } }
  end
  ...
  def self.get_details_for_multiple(incoming_request)
    group_data = UserGroup.get_attributes_for_multiple_by_id(incoming_request["ids"])
    return { :user_group => { :retrieved => group_data } }
  end
  ...
  def self.get_parent_user_groups(user_group_id)
    user_group = UserGroup.find_by_id(user_group_id)
    return { :user_group => { :deleted => [{ :id => user_group_id }] } } unless user_group

    parents = []
    user_groups = user_group.parent_user_groups
    parents = user_groups.map { |parent_user_group| parent_user_group.get_attributes } if user_groups
    return { :user_group => { :retrieved => parents } }
  end
  ...
  def self.get_library_item_tasks(item_id)
    ...
    task_array = LibraryItemTask.get_all_tasks_details("UserGroup", user_group.id)
  ...
  def self.update(user_group_id, incoming_request)
    ...
    return { :user_group => { :updated => [ user_group.get_attributes ] } }
  end
