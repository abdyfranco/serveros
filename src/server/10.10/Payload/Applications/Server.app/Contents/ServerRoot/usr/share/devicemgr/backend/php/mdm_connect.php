...
function _connect_transaction_1($incoming_request)
{
  ...
  case "Idle":
    ...
    $sql = "UPDATE mdm_tasks SET skipped = FALSE WHERE mdm_target_id = :mdm_target_id AND skipped = TRUE AND completed_at IS NULL";
    ExecuteSQL('mdm_tasks', $sql, ['mdm_target_id' => $tid]);
    ...
    if ($is_ls) {
      ...
      if (CV($target, 'hp_singleton_tasks') == 0 && CV($target, 'lp_singleton_tasks') == 0 && !Task_first_for_target($target)) {
        SaveToDatabase($target);          // Must update the row in the database because dm_update_all_profiles_for_mdm_target() uses some columns we've changed
        ExecuteSQLFunction('dm_update_all_profiles_for_mdm_target', ['target_id' => $tid]);
...
{
  $sync   = !empty($args[0]['NeedSyncResponse']);
  $target = $args[1];
  ReloadFromDatabase($target, 'FOR UPDATE');
  return Task_generate_next_request($target, $sync);
...
try {
  ...
  if ($gLogLevel >= 3) {
    ...
    LogMsg(basename(__FILE__).": incoming_request=$ir_str", 3);
  }
