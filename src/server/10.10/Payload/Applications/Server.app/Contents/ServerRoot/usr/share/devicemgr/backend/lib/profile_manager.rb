 ...
 def merge_payload_type_duplicates(payload_content)
      ...
      payload_content.each_with_index {|val, index|
          ...
          if first_index
              ...
              val.each {|key, value|
                  next if COMMON_PAYLOAD_KEYS.include?(key)
                  Rails.logger.error("Found duplicate values for key #{key} under payload type #{payload_type}") if existing_payload[key]
                  existing_payload[key] = value
              }
              payloads_to_delete.push(index)
          else
      ...
      rv = []
      payload_content.each_with_index {|val, index|
          rv.push(val) unless payloads_to_delete.include?(index)          
      }
