...
class MDMUtilities
  ...
  class MDMRetryTransaction < Exception
  end
  ...
  class MDMRollbackSilently < Exception
  end
  ...
  def self.process_in_transaction(name = nil, &block)
    ...
    return yield if @@in_transaction       # If we're already in a transaction, just yield to the block (if any)
    ...
    begin
      ...
      result = ActiveRecord::Base.transaction(&block)
      @@in_transaction = false
