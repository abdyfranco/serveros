...
module LibraryItemTaskHelper
  ...
  def self.find_all_active(remote_guid)
    return { :remote => {remote_guid.to_sym => []} } unless SettingsHelper.verify_od_apns
  ...
  def self.start_task(incoming_request, remote_guid)
    ...
    valid_target_types = ["User", "UserGroup", "Profile", "Device", "DeviceGroup"] | MDMUtilities.knob_set_class_names
    ...
    if valid_target_types.include?(target_class)
      ...
      u_session = User.session
      admin_guid = u_session && u_session[:user] && u_session[:user]['generated_uid']
      Rails.logger.error("start_task: admin_guid = #{admin_guid}")

      task = LibraryItemTask.create(target, task_type, incoming_request['params'], admin_guid, incoming_request['only_devices'])
      return (task ? { :library_item_task => { :created => [task.get_attributes] }, :remote => { remote_guid.to_sym => [ [:library_item_task, task.id] ] }  } : { :remote => {remote_guid.to_sym => []} })
    else
