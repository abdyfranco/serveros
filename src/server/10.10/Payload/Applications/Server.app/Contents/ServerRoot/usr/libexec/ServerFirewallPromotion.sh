...
grep "custom-firewall" /etc/pf.anchors/com.apple.server-firewall > /dev/null
PRESENT=$?
...
# We must also update combined_anchors.txt to include the missing static default rules.
# combined_anchors.txt is only used for reboot and is loaded by pfctl before the network stack is brought up.
# pfctl loads combined_anchors.txt in the rc.server script during boot.
