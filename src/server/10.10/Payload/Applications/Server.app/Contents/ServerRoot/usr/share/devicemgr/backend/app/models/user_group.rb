...
class UserGroup < ActiveRecord::Base
  ...
  def self.table_name
    return (Object.const_defined?('FinalizeMigration') ? 'user_groups' : 'abstract_user_groups')  # Can't use the new tables for v2m0 Finalize Migration
  end
  ...
  def self.get_extended_attributes_if_updated(selected_item_id, xmin, xip_ints, xip_sql)
  ...
  def self.parent_user_group_ids_for_user(user)
    ...
    sql = "SELECT id FROM view_active_user_parents WHERE user_id = #{user.id}"
  ...
  def self.search_for_user_groups_matching(search_string, max_results = nil, refresh_cache = nil)
    ...
    return self.connection.select_integer_values_by_index("SELECT id #{sql} ORDER BY LOWER(full_name) ASC, full_name DESC LIMIT #{max_results + 1}")
  end
  ...
  def self.search_for_vpp_enabled_user_groups_matching(search_string)
    ...
    return self.connection.select_integer_values_by_index("SELECT id #{sql} ORDER BY LOWER(full_name) ASC, full_name DESC")
  end
  ...
  def self.user_group_ids_for_admin_app
    ...
    sql = <<-SQL
      WITH cte AS (
        SELECT   id, COALESCE(full_name, short_name) AS sort
        FROM     view_all_valid_user_groups
      )
      SELECT   id
      FROM     cte
      ORDER BY LOWER(sort) ASC, sort DESC
      LIMIT    #{max_results + 1}
    SQL
    return self.connection.select_integer_values_by_index(sql)
  end
  ...
  def get_book_data
    sql = "SELECT dm_media_info_for_user_group(#{self.id})"
    json = self.connection.select_string_values_by_index(sql)   # This returns a JSON-formatted string because it's a complex structure that SQL can't represent easily
    return (json[0] && JSON.parse(json[0]))
  end
  ...
  def member_group_ids
    ...
    sql = <<-SQL
      WITH cte AS (
        SELECT   g.id, g.order_name AS sort
        FROM     view_all_valid_user_groups      AS g
  ...
  def member_user_ids
    ...
    sql = <<-SQL
      WITH cte AS (
        SELECT   u.id, u.order_name AS sort
  ...
  def modify_attributes(attr_hash, extended = false)
    ...
    sql = "SELECT id FROM view_active_user_groups WHERE id = #{self.id}"
    result = self.connection.select_integer_values_by_index(sql)
    ...
    if ((Settings.get_settings.od_sync_mode == 3) || is_active)
  ...
  def parent_user_group_ids
    sql = "SELECT id FROM view_active_user_group_parents WHERE child_id = #{self.id}"
    return self.connection.select_integer_values_by_index(sql)    
  end
