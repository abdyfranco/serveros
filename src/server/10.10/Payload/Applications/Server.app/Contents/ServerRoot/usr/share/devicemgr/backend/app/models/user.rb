...
class User < ActiveRecord::Base
  ...
  def self.table_name
    # We use the abstract table so we see all users. We never need to do inserts, and row updates will automatically target the correct table.
    return (Object.const_defined?('FinalizeMigration') ? 'users' : 'abstract_users')  # Can't use the new tables for v2m0 Finalize Migration
  end
  ...
  def self.search_for_users_matching(search_string, max_results = nil, refresh_cache = nil)
    ...
    return self.connection.select_integer_values_by_index("SELECT u.id #{sql} ORDER BY LOWER(order_name) ASC, order_name DESC LIMIT #{max_results + 1}")
  end
  ...
  def self.user_ids_for_admin_app
    ...
    sql = <<-SQL
      WITH cte AS (
        SELECT   id, COALESCE(full_name, last_name, first_name, short_name) AS sort
        FROM     view_all_valid_users
      )
      SELECT   id
      FROM     view_all_valid_users
      ORDER BY LOWER(order_name) ASC, order_name DESC
      LIMIT    #{max_results + 1}
    SQL
  ...
  def add_vpp_user_enrollment_info(attr_hash)
    ...
    if !has_content && attr_hash['app_data']
    ...
    if !has_content && attr_hash['book_data']
  ...
  def get_book_data
    sql = "SELECT dm_media_info_for_user(#{self.id})"
    json = self.connection.select_string_values_by_index(sql)   # This returns a JSON-formatted string because it's a complex structure that SQL can't represent easily
    return (json[0] && JSON.parse(json[0])) || []
  end
