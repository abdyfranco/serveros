...
module DataFileHelper
  ...
  def self.create_ebook_from_file(data_file_id, skip_push = false)
    ...
    file_data = title = nil
    root_path, file_name = self.extract_data_file_contents(['.epub', '.ibooks', '.pdf'], @@tmp_data_file.name, @@tmp_data_file.path)    # These are the acceptable final file/folder extensions we support, returns the actual file name found
    file_type = File.extname(file_name).downcase
  ...
  def self.find_eapp_icon(is_mac_app, info_plist, app_resources)
    ...
    if is_mac_app
      ...
      if info_plist['CFBundleIcons'] && info_plist['CFBundleIcons']['CFBundlePrimaryIcon'] && info_plist['CFBundleIcons']['CFBundlePrimaryIcon']['CFBundleIconFiles']
        info_plist['CFBundleIcons']['CFBundlePrimaryIcon']['CFBundleIconFiles'].each { |name|
