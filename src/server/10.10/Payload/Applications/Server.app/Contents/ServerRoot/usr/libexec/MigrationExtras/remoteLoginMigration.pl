...
## Migration Script for RemoteLogin, 10.6.x only!
...
#                   Time Machine backup.
# --sourceVersion <ver> The version number of the old system (like 10.6.8). Since we support
#                   migration from 10.6.8 - 10.10 installs.
...
#
# Example: /Applications/Server.app/Contents/ServerRoot/System/Library/ServerSetup/MigrationExtras/30-remoteLoginMigration.pl --purge 0 --language en --sourceType System --sourceVersion 10.6.8 --sourceRoot "/Library/Server/Previous" --targetRoot "/"
...
$VERSION = "";    # This is the version number of the old system passed into us by Server Assistant. [10.6.x, 10.7.x, 10.8.x, and potentially 10.9.x]
$PREV_EXT = ".previous";
...
sub restoreAndSetState()
{
    ...
    #We only care about this handling if we are comming from 10.6.x
	if( (${SRV_MINOR} eq "6") ) {
		if (-e ${OLD_LAUNCHD_OVERRIDES}) {
			if (${DEBUG}) {
				print("cmd := " . qq(${PLISTBUDDY} -c \"Print :com.openssh.sshd:Disabled\" \"${OLD_LAUNCHD_OVERRIDES}\") . "\n");
			} else {
				$RL_STATE=qx(${PLISTBUDDY} -c \"Print :com.openssh.sshd:Disabled\" \"${OLD_LAUNCHD_OVERRIDES}\"); chomp(${RL_STATE});
				if ( ($? eq "1") || ($? eq "256") ) {
					$RL_STATE="true"
				}
			}
...
################################################################################
sub startStopRemoteLogin()
{
	if (${FUNC_LOG}) {printf("startStopRemoteLogin := S\n");}
	
    #We only care about this handling if we are comming from 10.6.x
	if( (${SRV_MINOR} eq "6") ) {
		# No need to start if we aren't booted from the source drive.
		if(${gTargetRoot} eq "/") {
			qx(${LAUNCHCTL} load -w "${SSH_PLIST}");
			if (${DEBUG}) { printf("%s\n", qq(${LAUNCHCTL} load -w ${SSH_PLIST})); }
        }
	}

	if (${FUNC_LOG}) {printf("startStopRemoteLogin := E\n");}
}
...
sub Usage()
{
	...
	print("--sourceVersion <ver> The version number of the old system, 10.6.8." . "\n");
