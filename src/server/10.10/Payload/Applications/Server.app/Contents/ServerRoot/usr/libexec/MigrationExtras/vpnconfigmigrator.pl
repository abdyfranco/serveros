...
#                  Time Machine backup.
# --sourceVersion <ver> The version number of the old system (like 10.6 - 10.10). Since we support migration from 10.6
#                   through 10.10 installs.
...
sub startStopVPN()
{
	...
	#pptp
	if(${VPN_PPTP_DISABLED} eq "false") {
		$START_VPN=1;
		printf("PPTP is enabled\n");
	} else {
        printf("PPTP is disabled\n");
	}
	if($START_VPN == 1) {
