...
--      AND    task_type <> 'RemoveProfile'
  ...
  def self.create(library_item_target, task_type, parameters = nil, guid = nil, device_ids = nil)
    ...
    sql = "SELECT dm_create_basic_task_for_library_item('#{library_item_target.class.to_s}', #{library_item_target.id}, '#{task_type}', #{json}, #{guid}, #{targets}) AS id"
