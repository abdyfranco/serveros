...
class TargetTask < ActiveRecord::Base
  ...
  def self.table_name
    return "view_target_tasks"    # !!!READ-ONLY!!!
  end
  ...
  def self.get_updated(xmin, xip_ints, xip_sql)
    # Call dm_prepare_updated_target_tasks to populate the temp_updated_target_tasks table with the results we seek
    xip_sql = ", ARRAY[#{xip_sql}]" unless xip_sql == ''
    self.connection.execute("SELECT dm_prepare_updated_target_tasks(#{xmin}#{xip_sql})")
    return self.find_by_sql("SELECT * FROM temp_updated_target_tasks")
  end

  #-------------------------------------------------------------------------

  def before_save
    raise "TargetTask#before_save: read only!"
  end
  ...
  def delete
    raise "TargetTask#delete: read only!"
  end

  #-------------------------------------------------------------------------

  def destroy
    raise "TargetTask#destroy: read only!"
  end
