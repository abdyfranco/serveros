...
class MagicController < ApplicationController
  ...
  NonAdminModels = [ 'Asset', 'AssetMetadata', 'AutoJoinProfileUsage', 'EnterpriseApp', 'LibraryItemMetadata', 'Emailer', 'KnobSet', 'DeviceEnrollmentSettings', 'Ebook' ]
    ...
    begin
      ...
      incoming_request.each { |request_model, model_commands|
        ...
        model_commands.each { |command, params_array|
          ...
          AslUtility::log_user_agent_to_message_tracer("admin", request.headers['HTTP_USER_AGENT']) if model_helper == UserHelper && command == :find_all
          ...
          params_array.each { |entry|
            ...
            if model_helper == KnobSetHelper
              req_str = "#{model_helper}.#{command}(#{request_model},#{MDMLogger.debugOutput?(2) ? entry.join(",") : "..."})"
              ...
              req_str = "#{model_helper}.#{command}(#{MDMLogger.debugOutput?(2) ? entry.join(",") : "..."})"
              Rails.logger.info(">>>do_magic: #{req_str}")
              if MDMLogger.debugOutput?(2)
  ...
  def get_updated
    ...
    if xmin > 0      
      ...
      MagicController.admin_models.each { |model_class|
        ...
        if !found_selected && selected_item_class == model_str && model_class.respond_to?(:get_extended_attributes_if_updated)
        ...
        if updated_items.length > 0
          ...
          active_queries.each_pair { |guid, info|
            ...
            if model_str == selected_item_class && info['method'] == 'find_all'
              query_result = model_helper_class.send(info['method'], guid, selected_item_id)
            elsif model_str == selected_item_class && info['search_string'] && (info['method'] == 'find_matching' || info['method'] == 'find_matching_detailed')
